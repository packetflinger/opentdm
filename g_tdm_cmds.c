...
/**
 * Return true if the client has used a command that prints global info or
 * similar recently.
 */
qboolean TDM_RateLimited(edict_t *ent, int penalty) {
    if (level.realframenum < ent->client->resp.last_command_frame) {
        gi.cprintf(ent, PRINT_HIGH,
                "Command ignored due to rate limiting, wait a little longer.\n");
        return true;
    }
...
void TDM_Commands_f(edict_t *ent) {
    gi.cprintf(ent, PRINT_HIGH, "General\n"
            "-------\n"
...
/**
 * Show brief help on all admin/ref commands
 */
void TDM_Acommands_f(edict_t *ent) {
    gi.cprintf(ent, PRINT_HIGH, "Admin commands\n"
            "-------\n"
...
void TDM_Timelimit_f(edict_t *ent) {
    ...
    if (!input[0] || !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH, "Timelimit is %d minute%s.\n",
                (int) g_match_time->value / 60,
                (int) g_match_time->value / 60 == 1 ? "" : "s");
        return;
    }
    ...
    if (tdm_match_status >= MM_PLAYING && tdm_match_status < MM_SCOREBOARD) {
        ...
        level.match_end_framenum = level.match_start_framenum
...
void TDM_Powerups_f(edict_t *ent) {
    ...
    if (!input[0] || !ent->client->pers.admin) {
        for (i = 0; i < sizeof(powerupvotes) / sizeof(powerupvotes[0]); i++) {
            if ((int) g_powerupflags->value & powerupvotes[i].value) {
                strcat(settings,
                        TDM_SetColorText(va("%s", powerupvotes[i].names[0])));
                strcat(settings, " ");
            }
        }
    ...
    if (tdm_match_status > MM_COUNTDOWN && tdm_match_status < MM_SCOREBOARD) {
        gi.cprintf(ent, PRINT_HIGH,
                "You cannot change powerups during the match.\n");
        return;
    }
...
void TDM_Bfg_f(edict_t *ent) {
    ...
    if (tdm_match_status > MM_COUNTDOWN && tdm_match_status < MM_SCOREBOARD) {
        gi.cprintf(ent, PRINT_HIGH,
                "You cannot change bfg settings during the match.\n");
        return;
    }
    ...
    if (!input[0] || !ent->client->pers.admin) {
        if (g_tie_mode->value == 1) {
            gi.cprintf(ent, PRINT_HIGH, "Overtime is set to %d minute%s.\n",
                    (int) g_overtime->value / 60,
                    (int) g_overtime->value / 60 == 1 ? "" : "s");
        } else {
        ...
        if (!time[0]) {
            gi.cprintf(ent, PRINT_HIGH,
                    "Usage: overtime 0/1/2 <minutes> (tie/overtime/sudden death)\n");
            return;
        }
...
/**
 * Enable/disable observer talk during the match. values: speak,
 * whisper, shutup
 */
...
void TDM_Team_f(edict_t *ent) {
    ...
    if (gi.argc() < 2) {
        if (!ent->client->pers.team) {
            gi.cprintf(ent, PRINT_HIGH,
                    "You are not on a team. Use %s 1 or %s 2 to join a team.\n",
                    gi.argv(0), gi.argv(0));
            return;
        }
    ...
    if (tdm_match_status != MM_WARMUP) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can't change team in the middle of a match.\n");
        return;
    }
...
void TDM_Timeout_f(edict_t *ent) {
    ...
    if (!ent->client->pers.team && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only players in the match may call a time out.\n");
        return;
    }

    if ((int) g_timeout_captain->value
            && !(teaminfo[TEAM_A].captain == ent
                    || teaminfo[TEAM_B].captain == ent)) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team captains may call a time out.\n");
        return;
    }
    ...
    if (tdm_match_status == MM_TIMEOUT) {
        if (level.match_resume_framenum) {
            if (level.tdm_timeout_caller
                    && level.tdm_timeout_caller->client == ent) {
                gi.cprintf(ent, PRINT_HIGH,
                        "You may not call another time out.\n");
                return;
            }
            ...
            if (!level.tdm_timeout_caller) {
                ...
                if (!ent->client->pers.admin) {
                    gi.cprintf(ent, PRINT_HIGH,
                            "Only an admin can resume play.\n");
                    return;
                }
            } else if (level.tdm_timeout_caller->client == NULL) {
                gi.cprintf(ent, PRINT_HIGH,
                        "Match will automatically resume once %s reconnects.\n",
                        level.tdm_timeout_caller->name);
                return;
            } else if (level.tdm_timeout_caller->client != ent
                    && !ent->client->pers.admin) {
                gi.cprintf(ent, PRINT_HIGH,
                        "Only %s or an admin can resume play.\n",
                        level.tdm_timeout_caller->name);
                return;
            }
    ...
    if (tdm_match_status < MM_PLAYING || tdm_match_status == MM_SCOREBOARD) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can only call a time out during a match.\n");
        return;
    }
    ...
    if (g_timeout_limit->value && !ent->client->pers.admin) {
        if (ent->client->pers.timeout_count >= g_timeout_limit->value) {
            gi.cprintf(ent,
            PRINT_HIGH, "Timeout limit exceeded (%d/%d)",
    ...
    if (TDM_Is1V1()) {
        gi.bprintf(PRINT_CHAT,
                "%s called a time out. Match will resume automatically in %s.\n",
                ent->client->pers.netname, TDM_SecsToString(time_len));
    } else {
        gi.bprintf(PRINT_CHAT,
                "%s (%s) called a time out. Match will resume automatically in %s.\n",
                ent->client->pers.netname,
                teaminfo[ent->client->pers.team].name,
                TDM_SecsToString(time_len));
    }

    gi.cprintf(ent, PRINT_HIGH,
            "Match paused. Use '%s' again to resume play.\n", gi.argv(0));
}

/**
 * In 1v1 games, causes player who issues the command to win by forfeit. Only
 * used when opponent disconnects mid-match and this player doesn't want to
 * wait for them to reconnect.
 */
void TDM_Win_f(edict_t *ent) {
    if (!TDM_Is1V1()) {
        gi.cprintf(ent, PRINT_HIGH,
                "This command may only be used in 1v1 mode.\n");
        return;
    }

    if (tdm_match_status != MM_TIMEOUT || !level.tdm_timeout_caller
            || level.tdm_timeout_caller->client) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can only cause a forfeit during a time out caused by your opponent disconnecting.\n");
        return;
    }
...
char* TDM_SettingsString(void) {
    ...
    static const char *gamemode_text[] = { "Team Deathmatch",
            "Instagib Team Deathmatch", "1 vs 1 duel" };
    static const char *switchmode_text[] = { "normal", "faster", "instant",
            "insane", "extreme" };
    static const char *telemode_text[] = { "normal", "no freeze" };
    static const char *bugs_text[] = { "all gameplay bugs fixed",
            "serious gameplay bugs fixed", "no gameplay bugs fixed" };
    ...
    for (i = 0; i < sizeof(weaponvotes) / sizeof(weaponvotes[0]); i++) {
        if ((int) g_itemflags->value & weaponvotes[i].value) {
            strcat(settings,
                    TDM_SetColorText(va("%s", weaponvotes[i].names[1])));
            strcat(settings, " ");
        }
    }
    strcat(settings, "\n");
    ...
    strcat(settings,
            TDM_SetColorText(va("%s\n\n", bugs_text[(int) g_bugs->value])));

    strcat(settings, "Timeouts:         ");
    strcat(settings,
            TDM_SetColorText(
                    va("%s\n",
                            ((int) g_timeout_captain->value > 0) ?
                                    "captains only" : "all players")));
    ...
    if (TDM_Is1V1()) {
        ...
        if (mode == 0) {
            strcat(settings,
                    TDM_SetColorText(va("%s", "avoid closest (bugged)")));
        } else if (mode == 1) {
        ...
        if ((int) g_1v1_spawn_mode->value & SPAWN_RANDOM_ON_SMALL_MAPS) {
            strcat(settings,
                    TDM_SetColorText(va("%s", " (random on small maps)")));
        }
    } else {
...
void TDM_SV_ApplySettings_f(void) {
    if (g_gamemode->latched_string) { //can't use latched_string directly, cvar_forceset frees it
        gi.cvar_forceset("g_gamemode",
                va("%d", atoi(g_gamemode->latched_string)));
    }
...
/**
 * Show whether server is recording or not
 */
...
void TDM_Teamname_f(edict_t *ent) {
    ...
    if (g_gamemode->value == GAMEMODE_1V1) {
        gi.cprintf(ent, PRINT_HIGH,
                "This command is unavailable in 1v1 mode.\n");
        return;
    }
    ...
    if (teaminfo[ent->client->pers.team].captain != ent
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team captains or admins can change teamname.\n");
        return;
    }

    if (tdm_match_status != MM_WARMUP && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can only change team name during warmup.\n");
        return;
    }
    ...
    do {
        if (value[i] < 32 || value[i] == '"') {
            gi.cprintf(ent, PRINT_HIGH,
                    "Invalid team name, must not contain color text or quotes.\n");
            return;
        }
        i++;
    } while (value[i]);
...
void TDM_Lockteam_f(edict_t *ent, qboolean lock) {
    ...
    if (teaminfo[ent->client->pers.team].captain != ent
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team captains or admins can lock/unlock team.\n");
        return;
    }
...
void TDM_Invite_f(edict_t *ent) {
    ...
    if (teaminfo[ent->client->pers.team].captain != ent
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team captains or admins can invite players.\n");
        return;
    }
    ...
    if (tdm_match_status != MM_WARMUP && tdm_match_status != MM_PLAYING
            && tdm_match_status != MM_TIMEOUT) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can only invite players during warmup or during the match.\n");
        return;
    }
...
void TDM_PickPlayer_f(edict_t *ent) {
    ...
    if (teaminfo[ent->client->pers.team].captain != ent
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team captains or admins can pick players.\n");
        return;
    }
    ...
    if (tdm_match_status != MM_WARMUP && tdm_match_status != MM_PLAYING
            && tdm_match_status != MM_TIMEOUT) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can only pick players during warmup or during the match.\n");
        return;
    }
...
void TDM_Accept_f(edict_t *ent) {
    ...
    if (tdm_match_status != MM_WARMUP && tdm_match_status != MM_PLAYING
            && tdm_match_status != MM_TIMEOUT) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can only accept an invite during the match or warmup!\n");
    ...
    if (!ent->client->resp.last_invited_by->inuse
            || teaminfo[ent->client->resp.last_invited_by->client->pers.team].captain
                    != ent->client->resp.last_invited_by) {
    ...
    if ((g_max_players_per_team->value
            && teaminfo[ent->client->resp.last_invited_by->client->pers.team].players
                    >= g_max_players_per_team->value)
            || (tdm_match_status >= MM_PLAYING
                    && teaminfo[ent->client->resp.last_invited_by->client->pers.team].players
                            >= current_matchinfo.max_players_per_team)) {
    ...
    ent->client->pers.team =
            ent->client->resp.last_invited_by->client->pers.team;
    ...
    if (LookupPlayer(gi.argv(1), &victim, ent)) {
        ...
        if (tdm_match_status >= MM_PLAYING) {
            if (ent->client->pers.team == TEAM_SPEC
                    && victim->client->pers.team != TEAM_SPEC) {
                gi.cprintf(ent, PRINT_HIGH,
                        "Spectators cannot talk to players during the match.\n");
                return;
            }
        }

        if (g_chat_mode->value == 2 && ent->client->pers.team == TEAM_SPEC
                && !victim->client->pers.admin) {
            gi.cprintf(ent, PRINT_HIGH,
                    "Spectators cannot talk during shutup mode.\n");
            return;
        }
...
void TDM_KickPlayer_f(edict_t *ent) {
    ...
    if (teaminfo[ent->client->pers.team].captain != ent
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team captains or admins can kick players.\n");
        return;
    }
    ...
    if (LookupPlayer(name, &victim, ent)) {
        ...
        gi.cprintf(victim, PRINT_HIGH,
                "You were removed from team '%s' by %s.\n",
                teaminfo[victim->client->pers.team].name,
                ent->client->pers.netname);
...
void TDM_Admin_f(edict_t *ent) {
    if (ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "You are no longer registered as an admin.\n");
...
void TDM_Captain_f(edict_t *ent) {
    ...
    if (gi.argc() < 2) {
        ...
        if (ent->client->pers.team == TEAM_SPEC) {
            gi.cprintf(ent, PRINT_HIGH,
                    "You must join a team to set or become captain.\n");
            return;
        }
        ...
        if (teaminfo[ent->client->pers.team].captain == ent) {
            ...
            gi.cprintf(ent, PRINT_HIGH, "%s is the captain of team '%s'\n",
                    (teaminfo[ent->client->pers.team].captain)->client->pers.netname,
                    teaminfo[ent->client->pers.team].name);
        } else {
    ...
    //transferring captain to another player
        ...
        if (LookupPlayer(gi.argv(1), &victim, ent)) {
            ...
            gi.cprintf(victim, PRINT_HIGH,
                    "%s transferred captain status to you.\n",
                    ent->client->pers.netname);
        ...
        if (!ent->client->pers.admin) {
            gi.cprintf(ent, PRINT_HIGH,
                    "You must be admin to change another teams captain!\n");
            return;
        }
        team = TDM_GetTeamFromArg(ent, gi.argv(1));
...
void TDM_Captains_f(edict_t *ent) {
    gi.cprintf(ent, PRINT_HIGH,
            ...
            teaminfo[TEAM_A].captain ?
                    teaminfo[TEAM_A].captain->client->pers.netname : "(none)",
            teaminfo[TEAM_B].name,
            teaminfo[TEAM_B].captain ?
                    teaminfo[TEAM_B].captain->client->pers.netname : "(none)");
}
...
void TDM_Teamskin_f(edict_t *ent) {
    ...
    if (teaminfo[ent->client->pers.team].captain != ent
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team captains or admins can change teamskin.\n");
        return;
    }
    ...
    if (tdm_match_status != MM_WARMUP && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can only change team skin during warmup.\n");
        return;
    }
...
void TDM_Motd_f(edict_t *ent) {
    ...
    if (!g_motd_message || !g_motd_message->string[0]
            || ent->client->showmotd) {
    ...
    *string = 0;
    ...
    for (i = 0; i <= len; i++) {
        if (i == len || (message[i] == '\\' && message[i + 1] == 'n')) {
            ...
            if (current_length + strlen(message + msg_offset) + 23
                    > sizeof(string) - 1) {
                break;
            }

            sprintf(string + current_length, "xl 8 yb %d string \"%s\" ",
                    offset - 160, message + msg_offset);
            offset += 8;
            msg_offset = i + 2;
...
void TDM_Changeteamstatus_f(edict_t *ent, qboolean ready) {
    ...
    if (teaminfo[ent->client->pers.team].captain != ent
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team captains or admins can use teamready/teamnotready.\n");
        return;
    }
...
/**
 * Show the scoreboard from the last match.
 */
...
void TDM_Ghost_f(edict_t *ent) {
    ...
    if (ent->client->pers.team) {
        gi.cprintf(ent, PRINT_HIGH,
                "%s can only be used from spectator mode.\n", gi.argv(0));
        return;
    }
    ...
    if (!TDM_ProcessJoinCode(ent, code)) {
        gi.cprintf(ent, PRINT_HIGH,
                "No client information found for code %u.\n", code);
        return;
    }
}
...
void TDM_Mute_f(edict_t *ent) {
    ...
    if (LookupPlayer(gi.argv(1), &victim, ent)) {
        ...
        if (time > 0) {
            ...
            victim->client->pers.mute_frame = level.framenum
                    + SECS_TO_FRAMES(time * 60);
            gi.cprintf(ent, PRINT_HIGH, "%s is muted for %d minute%s\n",
                    victim->client->pers.netname, time, time == 1 ? "" : "s");
            gi.cprintf(victim, PRINT_HIGH,
                    "You are muted for %d minute%s by admin\n", time,
                    time == 1 ? "" : "s");
        }
    }
}
...
void TDM_Speclock_f(edict_t *ent) {
    ...
    if (!((int) g_command_mask->value & COMMAND_SPECLOCK)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Command '%s' is not allowed on this server.\n", gi.argv(0));
        return;
    }
    ...
    if (teaminfo[ent->client->pers.team].captain != ent
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team captains or admins can speclock team.\n");
        return;
    }
...
void TDM_Specinvite_f(edict_t *ent) {
    ...
    if (teaminfo[team].captain != ent && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team captains or admins can specinvite players.\n");
        return;
    }
    ...
    if (LookupPlayer(value, &victim, ent)) {
        ...
        if (victim->client->pers.specinvite[team]) {
            ...
            gi.cprintf(ent, PRINT_HIGH,
                    "%s was forbidden to spectate team '%s'.\n",
                    victim->client->pers.netname, teaminfo[team].name);
            gi.cprintf(victim, PRINT_HIGH,
                    "You were forbidden by %s to spectate team '%s'.\n",
                    ent->client->pers.netname, teaminfo[team].name);
        } else {
            victim->client->pers.specinvite[team] = true;
            gi.cprintf(ent, PRINT_HIGH,
                    "%s was invited to spectate team '%s'.\n",
                    victim->client->pers.netname, teaminfo[team].name);
            gi.cprintf(victim, PRINT_HIGH,
                    "You were invited by %s to spectate team '%s'.\n",
                    ent->client->pers.netname, teaminfo[team].name);
        }
    }
}
...
/**
 * Move to spectator or change spectating mode
 */
void TDM_Spectate_f(edict_t *ent) {
    ...
    } else if (!Q_stricmp(gi.argv(1), "invul") || !Q_stricmp(gi.argv(1), "pent")
            || !Q_stricmp(gi.argv(1), "666")) {
        ent->client->resp.spec_mode = SPEC_INVUL;
    } else if (!Q_stricmp(gi.argv(1), "killer")) {
        ent->client->resp.spec_mode = SPEC_KILLER;
    } else if (!Q_stricmp(gi.argv(1), "leader")
            || !Q_stricmp(gi.argv(1), "topfragger")) {
        ent->client->resp.spec_mode = SPEC_LEADER;
    } else {
        gi.cprintf(ent, PRINT_HIGH,
                "Usage: %s [quad/invul/killer/leader/none]\n", gi.argv(0));
        return;
    }
...
void TDM_TeamEnemySkin_f(edict_t *ent, qboolean team) {
    ...
    if (team) {
        Q_strncpy(ent->client->pers.config.teamskin, gi.argv(1),
                sizeof(ent->client->pers.config.teamskin) - 1);
    } else {
        Q_strncpy(ent->client->pers.config.enemyskin, gi.argv(1),
                sizeof(ent->client->pers.config.enemyskin) - 1);
    }
...
void TDM_PlayerConfigDisplay_f(edict_t *ent) {
    ...
    if (!ent->client->pers.config.loaded) {
        gi.cprintf(ent, PRINT_HIGH,
                "No specific playerconfig is loaded for you, using defaults.\n");
        return;
    }

    url = va("http://%s%s%s", g_http_domain->string, g_http_path->string,
...
void TDM_ArmorTimer_f(edict_t *ent) {
    ...
    if (gi.argc() == 2 && !Q_stricmp(gi.argv(1), "-h")) {
        gi.cprintf(ent, PRINT_HIGH, "%s: %s - Start an armor timer\n"
                "       %s [%s] - set your auto-timing mask\n\n"
                "%s contains any: [-/+]all [-/+]ra [-/+]ya [-/+]ga\n\n",
                "Usage", gi.argv(0), gi.argv(0),
    ...
    if (g_armor_timer->value) {
        ...
        if (gi.argc() < 2) {
            ent->client->pers.item_timer[TIMER_ARMOR] = level.framenum
                    + SECS_TO_FRAMES(20);
            ent->client->pers.item_timer_icon[TIMER_ARMOR] = gi.imageindex(
                    "i_combatarmor");
        } else {
            mask = gi.argv(1);
        ...
        if (((int) g_vote_mask->value) & VOTE_ARMOR_TIMER) {
            gi.cprintf(ent, PRINT_HIGH,
                    "Armor timer disabled, vote to enable it: \"vote armortimer 1\"\n");
        } else {
            gi.cprintf(ent, PRINT_HIGH,
                    "Armor timer disabled in server config\n");
        }
    }
}
...
void TDM_WeaponTimer_f(edict_t *ent) {
    ...
    if (g_weapon_timer->value) {
        // command used alone, just start a timer
        if (gi.argc() < 2) {
            ent->client->pers.item_timer[TIMER_WEAPON] = level.framenum
                    + SECS_TO_FRAMES(30);
            ent->client->pers.item_timer_icon[TIMER_WEAPON] = gi.imageindex(
                    "w_blaster");
        } else {
            mask = gi.argv(1);
            if (mask[0] >= '0' && mask[0] <= '9') { // assume it's a numeric mask
        ...
        if (((int) g_vote_mask->value) & VOTE_WEAPON_TIMER) {
            gi.cprintf(ent, PRINT_HIGH,
                    "Weapon timer disabled, vote to enable it: \"vote weapontimer 1\"\n");
        } else {
            gi.cprintf(ent, PRINT_HIGH,
                    "Weapon timer disabled in server config\n");
        }
    }
}
...
qboolean TDM_Command(const char *cmd, edict_t *ent) {
    if (ent->client->pers.admin) {
        if (!Q_stricmp(cmd, "forceready") || !Q_stricmp(cmd, "readyall")
                || !Q_stricmp(cmd, "allready")
                || !Q_stricmp(cmd, "startcountdown")) {
            ...
            TDM_Acommands_f(ent);
            return true;
        } else if (!Q_stricmp(cmd, "unreadyall")
                || !Q_stricmp(cmd, "notreadyall")) {
    ...
    if (tdm_match_status == MM_TIMEOUT || tdm_match_status == MM_SCOREBOARD) {
        if (!Q_stricmp(cmd, "commands")) {
            ...
            TDM_Settings_f(ent);
        } else if (!Q_stricmp(cmd, "calltime") || !Q_stricmp(cmd, "pause")
                || !Q_stricmp(cmd, "ctime") || !Q_stricmp(cmd, "time")
                || !Q_stricmp(cmd, "hold")) {
            TDM_Timeout_f(ent);
        } else if (!Q_stricmp(cmd, "ghost") || !Q_stricmp(cmd, "restore")
                || !Q_stricmp(cmd, "recover") || !Q_stricmp(cmd, "rejoin")) {
            TDM_Ghost_f(ent);
        } else if (!Q_stricmp(cmd, "win")) {
            TDM_Win_f(ent);
        } else if (!Q_stricmp(cmd, "observer") || !Q_stricmp(cmd, "spectate")
                || !Q_stricmp(cmd, "chase") || !Q_stricmp(cmd, "spec")
                || !Q_stricmp(cmd, "obs")) {
            TDM_Spectate_f(ent);
        ...
        if (!Q_stricmp(cmd, "ready")) {
            TDM_Ready_f(ent);
                ...
                || !Q_stricmp(cmd, "noready")) {
            TDM_NotReady_f(ent);
        } else if (!Q_stricmp(cmd, "kickplayer")
                || !Q_stricmp(cmd, "removeplayer") || !Q_stricmp(cmd, "remove")
                || !Q_stricmp(cmd, "k")) {
            TDM_KickPlayer_f(ent);
        } else if (!Q_stricmp(cmd, "admin") || !Q_stricmp(cmd, "referee")) {
            TDM_Admin_f(ent);
        ...
        } else if (!Q_stricmp(cmd, "teamnotready")
        ...
        // stats.. this could use some cleanup
                ...
                || !Q_stricmp(cmd, "accuracy") || !Q_stricmp(cmd, "damage")
                || !Q_stricmp(cmd, "weapons") || !Q_stricmp(cmd, "items")
            ...
            TDM_Stats_f(ent, &current_matchinfo);
                ...
                || !Q_stricmp(cmd, "lastaccuracy")
                || !Q_stricmp(cmd, "olddamage") || !Q_stricmp(cmd, "lastdamage")
                || !Q_stricmp(cmd, "oldweapons")
                || !Q_stricmp(cmd, "lastweapons") || !Q_stricmp(cmd, "olditems")
                || !Q_stricmp(cmd, "lastitems")
                || !Q_stricmp(cmd, "oldkillstats")
                || !Q_stricmp(cmd, "lastkillstats")
            ...
            TDM_Stats_f(ent, &old_matchinfo);
                ...
                || !Q_stricmp(cmd, "teamweapons")
                || !Q_stricmp(cmd, "teamitems")
                || !Q_stricmp(cmd, "teamkillstats")
                || !Q_stricmp(cmd, "teamdeathstats")) {
            TDM_TeamStats_f(ent, &current_matchinfo);
        } else if (!Q_stricmp(cmd, "oldteamstats")
                || !Q_stricmp(cmd, "oldteamkills")
                || !Q_stricmp(cmd, "lastteamstats")
                || !Q_stricmp(cmd, "lastteamkills")
                || !Q_stricmp(cmd, "oldteamaccuracy")
                || !Q_stricmp(cmd, "lastteamaccuracy")
                || !Q_stricmp(cmd, "oldteamdamage")
                || !Q_stricmp(cmd, "lastteamdamage")
                || !Q_stricmp(cmd, "oldteamweapons")
                || !Q_stricmp(cmd, "lastteamweapons")
                || !Q_stricmp(cmd, "oldteamitems")
                || !Q_stricmp(cmd, "lastteamitems")
                || !Q_stricmp(cmd, "oldteamkillstats")
                || !Q_stricmp(cmd, "lastteamkillstats")
                || !Q_stricmp(cmd, "lastteamdeathstats")
        ...
        } else if (!Q_stricmp(cmd, "oldscores") || !Q_stricmp(cmd, "oldscore")
                || !Q_stricmp(cmd, "lastscores")
                || !Q_stricmp(cmd, "lastscore")) {
            TDM_OldScores_f(ent);
        } else if (!Q_stricmp(cmd, "ghost") || !Q_stricmp(cmd, "restore")
                || !Q_stricmp(cmd, "recover") || !Q_stricmp(cmd, "rejoin")) {
            TDM_Ghost_f(ent);
