...
void TDM_ApplyVote(void) {
    ...
    if (vote.flags
            & (
            VOTE_TIMELIMIT | VOTE_WEAPONS | VOTE_POWERUPS | VOTE_GAMEMODE
                    | VOTE_TELEMODE | VOTE_TIEMODE |
                    VOTE_SWITCHMODE | VOTE_OVERTIME | VOTE_CONFIG
                    | VOTE_WEBCONFIG | VOTE_CHAT | VOTE_BUGS |
        ...
        if (vote.gamemode == GAMEMODE_1V1) {
            ...
            for (i = 1; i <= game.maxclients; i++) {
                ...
                if (ent->client->pers.team
                        && teaminfo[ent->client->pers.team].captain != ent) {
    ...
    if (vote.flags & VOTE_TIMEOUT_LIMIT) {
        ...
        g_timeout_limit = gi.cvar_set("g_timeout_limit",
                va("%d", vote.timeoutlimit));
    ...
    if (vote.flags & VOTE_TIMEOUT_CAPTAIN) {
        ...
        g_timeout_captain = gi.cvar_set("g_timeout_captain",
                va("%d", vote.timeoutcaptain));
        gi.bprintf(PRINT_HIGH, "%s\n",
                (vote.timeoutcaptain) ?
                        "Captains only can call timeout" :
                        "Any player can call timeout");
    }

    if (vote.flags & VOTE_ABORT) {
        abort: gi.bprintf(PRINT_CHAT, "Match aborted.\n");
        TDM_EndMatch();
    }
    ...
    *vote_string = 0;
    ...
    if (vote.active) {
        ...
        if (strlen(vote_string) > 63) {
            sprintf(vote_string,
                    "Vote: type 'vote' to see changes. Yes: %d (%d) No: %d [%02d]",
...
/**
 * Announce vote to other players. should be on screen vote
 */
static void TDM_AnnounceVote(void) {
    ...
    if (vote.flags & VOTE_WEAPONS) {
        ...
        for (j = 0; j < sizeof(weaponvotes) / sizeof(weaponvotes[1]); j++) {
            if ((vote.newweaponflags & weaponvotes[j].value)
                    && !((int) g_itemflags->value & weaponvotes[j].value)) {
                strcat(what, va(" -%s", weaponvotes[j].names[1]));
            } else if (!(vote.newweaponflags & weaponvotes[j].value)
                    && ((int) g_itemflags->value & weaponvotes[j].value)) {
                strcat(what, va(" +%s", weaponvotes[j].names[1]));
            }
        }
    ...
    if (vote.flags & VOTE_POWERUPS) {
        ...
        for (j = 0; j < sizeof(powerupvotes) / sizeof(powerupvotes[1]); j++) {
            if ((vote.newpowerupflags & powerupvotes[j].value)
                    && !((int) g_powerupflags->value & powerupvotes[j].value)) {
                strcat(what, va(" -%s", powerupvotes[j].names[0]));
            } else if (!(vote.newpowerupflags & powerupvotes[j].value)
                    && ((int) g_powerupflags->value & powerupvotes[j].value)) {
                strcat(what, va(" +%s", powerupvotes[j].names[0]));
            }
        }
    ...
    if (g_vote_attention->value) {
        gi.sound(world, 0, gi.soundindex(g_vote_attention_sound->string), 1,
                ATTN_NONE, 0);
    }
}
...
qboolean TDM_VoteTimeLimit(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_TIMELIMIT)
        ...
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for timelimit is not allowed on this server.\n");
        return false;
    }
...
qboolean TDM_VoteMap(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_MAP) && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for map is not allowed on this server.\n");
        return false;
    }
...
qboolean TDM_VoteWeapons(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_WEAPONS)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for weapons is not allowed on this server.\n");
        return false;
    }
    ...
    if ((flags & WEAPON_BFG10K) && (flags & WEAPON_HYPERBLASTER)
            && ((unsigned) g_powerupflags->value & POWERUP_POWERSCREEN)
            && ((unsigned) g_powerupflags->value & POWERUP_POWERSHIELD)) {
        flags |= AMMO_CELLS;
    } else {
        flags &= ~AMMO_CELLS;
    }
...
qboolean TDM_VoteKick(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_KICK) && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for player kick is not allowed on this server.\n");
        return false;
    }
...
qboolean TDM_VotePowerups(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_POWERUPS)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for powerups is not allowed on this server.\n");
        return false;
    }
    ...
    if (!value[0]) {
        gi.cprintf(ent, PRINT_HIGH,
                "Usage: vote powerups <+/-><quad,invul,ps,..>\n");
        return false;
    }
    ...
    for (i = 2; i < gi.argc(); i++) {
        ...
        // consider 1 as +all and 0 as -all
...
/**
 * Vote to change the game mode. Game mode is overall 'style' of the mod and a
 * change of this will cause a dump of all clients back to spectator mode and
 * menu.
 */
qboolean TDM_VoteGameMode(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_GAMEMODE)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for game mode is not allowed on this server.\n");
        return false;
    }
...
qboolean TDM_VoteTieMode(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_TIEMODE)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for tie mode is not allowed on this server.\n");
        return false;
    }
    ...
    if (!Q_stricmp(value, "ot") || !Q_stricmp(value, "overtime")) {
        ...
        tiemode = 2;
    } else if (!Q_stricmp(value, "none") || !Q_stricmp(value, "tie")
            || !Q_stricmp(value, "normal")) {
        tiemode = 0;
    } else {
...
qboolean TDM_VoteTeleMode(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_TELEMODE)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for teleporter mode is not allowed on this server.\n");
        return false;
    }
...
qboolean TDM_VoteSwitchMode(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_SWITCHMODE)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for weapon switch mode is not allowed on this server.\n");
        return false;
    }
    ...
    if (!Q_stricmp(value, "fast") || !Q_stricmp(value, "faster")) {
        ...
        switchmode = 4;
            ...
            || !Q_stricmp(value, "default") || !Q_stricmp(value, "q2")) {
        switchmode = 0;
    } else {
    ...
    if (g_fast_weap_switch->value == switchmode) {
        gi.cprintf(ent, PRINT_HIGH,
                "That weapon switch mode is already set!\n");
        return false;
    }
...
qboolean TDM_VoteOverTimeLimit(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_OVERTIME)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for overtime is not allowed on this server.\n");
        return false;
    }
...
void TDM_CreateConfiglist(void) {
    ...
    if (j == 0) {
        ...
        for (i = 0; tdm_configlist[i] != NULL; i++) {
            if (strlen(tdm_configlist[i]) + j
                    >= sizeof(tdm_configlist_string) - 16) {
...
/**
 * Vote a config file. A config file is essentially a set of votables set to
 * certain values that all get applied at once.
 */
qboolean TDM_VoteConfig(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_CONFIG) && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for config is not allowed on this server.\n");
        return false;
    }
    ...
    Com_sprintf(path, sizeof(path), "./%s/configs/%s", game.gamedir,
            configname);
...
/**
 * Start a web config download. Web configs are configs stored on opentdm.org
 * that are downloaded via libcurl, allowing a single player / tournament
 * config to be used by every OpenTDM server.
 */
qboolean TDM_VoteWebConfig(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_WEBCONFIG)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for web config is not allowed on this server.\n");
        return false;
    }
    ...
    if (tdm_vote_download.inuse) {
        gi.cprintf(ent, PRINT_HIGH,
                "Another config download is pending, please try again later.\n");
        return false;
    }

    Com_sprintf(tdm_vote_download.path, sizeof(tdm_vote_download.path),
            "%s.cfg", value);
    tdm_vote_download.initiator = ent;
    tdm_vote_download.type = DL_CONFIG;
    Q_strncpy(tdm_vote_download.name, value,
            sizeof(tdm_vote_download.name) - 1);
...
/**
 * A web config finished downloading. Be very careful here as the game or
 * player state may have changed between when the config was requested and when
 * we get called! Note, the download core checks if the player disconnected, so
 * we don't need to worry about things like that - only game state.
 */
void TDM_VoteWebConfigResult(edict_t *ent, int code, tdm_config_t *config) {
    ...
    if (code == 404) {
        gi.cprintf(ent, PRINT_HIGH,
                "Web config '%s' was not found. Visit opentdm.org for more information on web configs.\n",
                tdm_vote_download.name);
        return;
    } else if (code == 600) {
        gi.cprintf(ent, PRINT_HIGH,
                "Unable to parse the config file. It may be corrupt.\n");
        return;
    }

    if (!config) {
        gi.cprintf(ent, PRINT_HIGH,
                "The OpenTDM web config service is unavailable at the moment.\n");
        return;
    }

    if (tdm_match_status != MM_WARMUP) {
        gi.cprintf(ent, PRINT_HIGH,
                "Web config '%s' was found, but it is too late to propose settings now.\n",
                tdm_vote_download.name);
        return;
    }
    ...
    if (!ent->client->pers.team && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Web config '%s' was found, but you are no longer on a team!\n",
                tdm_vote_download.name);
        return;
    }
    ...
    if (vote.active) {
        gi.cprintf(ent, PRINT_HIGH,
                "Web config '%s' was found, but another vote has already started.\n",
                tdm_vote_download.name);
        return;
    }
...
qboolean TDM_VoteChat(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_CHAT) && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for chat mode is not allowed on this server.\n");
        return false;
    }
    ...
    if (!Q_stricmp(value, "players") || !Q_stricmp(value, "team")
            || !Q_stricmp(value, "nospec") || !Q_stricmp(value, "whisper")
            || !Q_stricmp(value, "1")) {
        chatmode = 1;
    } else if (!Q_stricmp(value, "all") || !Q_stricmp(value, "everyone")
            || !Q_stricmp(value, "speak") || !Q_stricmp(value, "0")) {
        chatmode = 0;
    } else {
...
qboolean TDM_VoteRestart(edict_t *ent) {
    if (!((int) g_vote_mask->value & VOTE_RESTART)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for match restart is not allowed on this server.\n");
        return false;
    }
    ...
    if (!ent->client->pers.team && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only players in the match can vote for a restart.\n");
        return false;
    }

    if (tdm_match_status == MM_TIMEOUT && TDM_Is1V1()
            && level.tdm_timeout_caller && !level.tdm_timeout_caller->client) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can't restart a 1v1 match without your opponent present. If you don't wish to wait, type \"win\" in the console to force the match to end.\n");
        return false;
    }
...
qboolean TDM_VoteAbort(edict_t *ent) {
    if (!((int) g_vote_mask->value & VOTE_ABORT) && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for match abort is not allowed on this server.\n");
        return false;
    }
    ...
    if (!ent->client->pers.team && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only players in the match can vote for an abort.\n");
        return false;
    }
...
qboolean TDM_VoteBugs(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_BUGS) && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for gameplay bugs is not allowed on this server.\n");
        return false;
    }
    ...
    if ((int) g_bugs->value == bugs) {
        gi.cprintf(ent, PRINT_HIGH,
                "That gameplay bugs settings is already set!\n");
        return false;
    }
...
qboolean TDM_VoteShuffle(edict_t *ent) {
    if (!((int) g_vote_mask->value & VOTE_SHUFFLE)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for player shuffling is not allowed on this server.\n");
        return false;
    }
    ...
    if (!ent->client->pers.team && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only team players can vote for a shuffle.\n");
        return false;
    }
...
qboolean TDM_VoteArmorTimer(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_ARMOR_TIMER)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting armor timer is not allowed in server config\n");
        return false;
    }
...
qboolean TDM_VoteWeaponTimer(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_WEAPON_TIMER)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting weapon timer is not allowed in server config\n");
        return false;
    }
...
qboolean TDM_VoteTimeoutCaptain(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_TIMEOUT_CAPTAIN)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting timeout behavior is not allowed in server config\n");
        return false;
    }
...
qboolean TDM_VoteTimeoutLimit(edict_t *ent) {
    ...
    if (!((int) g_vote_mask->value & VOTE_TIMEOUT_LIMIT)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting timeout limits is not allowed in the server config\n");
        return false;
    }
...
void TDM_Vote_f(edict_t *ent) {
    ...
    if ((!Q_stricmp(gi.argv(0), "yes") || !Q_stricmp(gi.argv(0), "no"))
            && (!vote.active
                    || (!ent->client->pers.team && !ent->client->pers.admin))) {
    ...
    if (!Q_stricmp(gi.argv(0), "yes") || !Q_stricmp(gi.argv(0), "no")) {
        ...
        if (gi.argc() < 2) {
            ...
            gi.cprintf(ent, PRINT_HIGH, "Usage: vote <setting> <value>\n"
                    " Options:\n"
                    "  timelimit <minutes>\n"
                    "  map <mapname>\n"
                    "  kick <player/id>\n"
    ...
    if (!(int) g_vote_mask->value && !ent->client->pers.admin
            && Q_stricmp(cmd, "yes") && Q_stricmp(cmd, "no")) {
        gi.cprintf(ent, PRINT_HIGH,
                "Proposing new settings is not allowed on this server.\n");
        return;
    }
    ...
    if (tdm_match_status != MM_WARMUP) {
        if (!Q_stricmp(cmd, "timelimit") || !Q_stricmp(cmd, "tl")
                || !Q_stricmp(cmd, "restart") || !Q_stricmp(cmd, "yes")
                || !Q_stricmp(cmd, "no") || !Q_stricmp(cmd, "kick")
                || !Q_stricmp(cmd, "abort")) {
            if (!(tdm_match_status >= MM_PLAYING
                    && tdm_match_status < MM_SCOREBOARD)) {
                gi.cprintf(ent, PRINT_HIGH,
                        "You can only vote for a timelimit change or restart during a match.\n");
                return;
            }
        } else {
            gi.cprintf(ent, PRINT_HIGH,
                    "You can only propose new settings during warmup.\n");
            return;
        }
    }
    ...
    if (!ent->client->pers.team && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "You must be on a team to vote or propose new settings.\n");
        return;
    }

    //if initiator wants to change vote reset the vote and start again
    if (vote.active && vote.initiator != ent && Q_stricmp(cmd, "yes")
            && Q_stricmp(cmd, "no")) {
    ...
    if (!ent->client->pers.admin
            && ent
                    == vote.last_initiator&& level.framenum - vote.last_vote_end_frame < SECS_TO_FRAMES(10)) {
        gi.cprintf(ent, PRINT_HIGH,
                "You must wait a short time before proposing another vote.\n");
        return;
    }
...
void TDM_VoteMenuApply(edict_t *ent) {
    ...
    if (tdm_match_status != MM_PLAYING && tdm_match_status != MM_WARMUP) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can propose new settings only during warmup.\n");
        return;
    }

    if (ent->client->pers.votemenu_values.timelimit
            != ((unsigned) g_match_time->value / 60)) {
    ...
    if (ent->client->pers.votemenu_values.map[0] != '\0'
            && ent->client->pers.votemenu_values.map[0] != '-'
    ...
    if ((ent->client->pers.votemenu_values.bfg
            && ((int) g_itemflags->value & WEAPON_BFG10K))
            || (!ent->client->pers.votemenu_values.bfg
                    && (~(int) g_itemflags->value & WEAPON_BFG10K))) {
    ...
    if ((ent->client->pers.votemenu_values.powerups
            && (int) g_powerupflags->value != 0)
            || (!ent->client->pers.votemenu_values.powerups
                    && !(int) g_powerupflags->value)) {
        if (ent->client->pers.votemenu_values.powerups) {
    ...
    if (ent->client->pers.votemenu_values.chat
            != (unsigned) g_chat_mode->value) {
    ...
    if (ent->client->pers.votemenu_values.gamemode
            != (unsigned) g_gamemode->value) {
    ...
    if (ent->client->pers.votemenu_values.config[0]
            && ent->client->pers.votemenu_values.config[0] != '-') {
        if (newvote) {
            gi.cprintf(ent, PRINT_HIGH,
                    "You cannot propose vote config with other options.\n");
        } else {
    ...
    if (ent->client->pers.votemenu_values.kick != NULL) {
        if (newvote) {
            gi.cprintf(ent, PRINT_HIGH,
                    "You cannot propose vote kick with other options.\n");
        } else {
...
qboolean TDM_ParseVoteConfigLine(char *line, int line_number, void *param) {
    ...
    if (!strcmp(variable, "timelimit")) {
        ...
        gi.dprintf(
                "WARNING: Unknown variable '%s' on line %d of web config. Check you are using the latest version of OpenTDM.\n",
 ...
 */
void TDM_ConfigDownloaded(tdm_download_t *download, int code, byte *buff,
        int len) {
    tdm_config_t *t, *last;
...
int TDM_WeaponStringToBitmask(const char *str) {
    ...
    while (token) {
        ...
        for (i = 0; i < WEAPON_MAX; i++) {
            ...
            if (!Q_stricmp(token, weaponvotes[i].names[0])
                    || !Q_stricmp(token, weaponvotes[i].names[1])) {
                if (modifier == '+') {
...
int TDM_ArmorStringToBitmask(const char *str) {
    ...
    while (token) {
        ...
        for (i = 0; i < ARMOR_MAX; i++) {
            ...
            if (!Q_stricmp(token, armorvotes[i].names[0])
                    || !Q_stricmp(token, armorvotes[i].names[1])) {
                if (modifier == '+') {
