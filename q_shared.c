...
void Info_SetValueForKey(char *s, const char *key, const char *value) {
    ...
    if (strchr(key, '\\') || strchr(value, '\\')) {
        Com_Printf(
                "Can't use keys or values with a \\ (attempted to set key '%s')\n",
                LOG_GENERAL, key);
        return;
    }
    ...
    if (strchr(key, ';') || strchr(value, ';')) {
        Com_Printf(
                "Can't use keys or values with a semicolon (attempted to set key '%s')\n",
                LOG_GENERAL, key);
        return;
    }
    ...
    if (strlen(key) > MAX_INFO_KEY - 1 || strlen(value) > MAX_INFO_KEY - 1) {
        Com_Printf(
                "Keys and values must be < 64 characters (attempted to set key '%s')\n",
                LOG_GENERAL, key);
        return;
    }
