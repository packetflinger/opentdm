...
void P_DamageFeedback(edict_t *player) {
    ...
    if ((level.framenum > player->pain_debounce_framenum)
...
void SV_CalcViewOffset(edict_t *ent) {
    ...
    if (ent->deadflag) {
        ...
        if ((level.framenum % (int) (0.1f * SERVER_FPS)) == 0) {
            ...
            ratio = (ent->client->v_dmg_framenum - level.framenum)
                    / (float) DAMAGE_FRAMES;
            if (ratio < 0) {
                ratio = 0;
            ...
            ratio = (ent->client->fall_framenum - level.framenum)
                    / (float) FALL_FRAMES;
            if (ratio < 0) {
                ratio = 0;
            }
...
void SV_CalcBlend(edict_t *ent) {
    ...
    if (ent->client->quad_framenum > level.framenum) {
        ...
        if (remaining == SECS_TO_FRAMES(3)) {    // beginning to fade
            gi.sound(ent, CHAN_ITEM, gi.soundindex("items/damage2.wav"), 1,
                    ATTN_NORM, 0);
        }
        if (remaining > SECS_TO_FRAMES(3)
                || ((remaining / SECS_TO_FRAMES(0.1f)) & 4)) {
            SV_AddBlend(0, 0, 1, 0.08f, ent->client->ps.blend);
        }
    } else if (ent->client->invincible_framenum > level.framenum) {
        remaining = ent->client->invincible_framenum - level.framenum;
        if (remaining == SECS_TO_FRAMES(3)) {   // beginning to fade
            gi.sound(ent, CHAN_ITEM, gi.soundindex("items/protect2.wav"), 1,
                    ATTN_NORM, 0);
        }
        if (remaining > SECS_TO_FRAMES(3)
                || ((remaining / SECS_TO_FRAMES(0.1f)) & 4)) {
        ...
        if (remaining == SECS_TO_FRAMES(3)) {   // beginning to fade
            gi.sound(ent, CHAN_ITEM, gi.soundindex("items/airout.wav"), 1,
                    ATTN_NORM, 0);
        }
        if (remaining > SECS_TO_FRAMES(3)
                || ((remaining / SECS_TO_FRAMES(0.1f)) & 4)) {
...
void P_FallingDamage(edict_t *ent) {
    ...
    if (delta > 30) {
        if (ent->health > 0) {
            if (delta >= 55) {
                gi.sound(ent, CHAN_AUTO, soundcache[SND_FALL1], 1, ATTN_NORM,
                        0);
            } else {
                gi.sound(ent, CHAN_AUTO, soundcache[SND_FALL2], 1, ATTN_NORM,
                        0);
            }
        }
        ...
        if (!((int) dmflags->value & DF_NO_FALLING)) {
            T_Damage(ent, world, world, dir, ent->s.origin, vec3_origin, damage,
                    0, 0, MOD_FALLING);
        }
    } else {
...
void P_WorldEffects(void) {
    ...
    if (current_player->movetype == MOVETYPE_NOCLIP) {
        current_player->air_finished_framenum = level.framenum
                + SECS_TO_FRAMES(12);   // don't need air
        return;
    }
    ...
    if (old_waterlevel == 3 && waterlevel != 3) {
        if (current_player->air_finished_framenum < level.framenum) {   // gasp for air
            gi.sound(current_player, CHAN_VOICE,
                    gi.soundindex("player/gasp1.wav"), 1, ATTN_NORM, 0);
        } else if (current_player->air_finished_framenum
                < level.framenum + SECS_TO_FRAMES(11)) {    // just break surface
            gi.sound(current_player, CHAN_VOICE,
    ...
    if (waterlevel == 3) {
        ...
        if (breather || envirosuit) {
            ...
            if (((current_client->breather_framenum - level.framenum)
                    % (25 * SECS_TO_FRAMES(0.1f))) == 0) {
                if (!current_client->breather_sound) {
                    gi.sound(current_player, CHAN_AUTO,
                            gi.soundindex("player/u_breath1.wav"), 1, ATTN_NORM,
                            0);
                } else {
                    gi.sound(current_player, CHAN_AUTO,
                            gi.soundindex("player/u_breath2.wav"), 1, ATTN_NORM,
                            0);
                }
        ...
        if (current_player->air_finished_framenum < level.framenum) {// drown!
            if (current_player->client->next_drown_framenum < level.framenum
                    && current_player->health > 0) {
                current_player->client->next_drown_framenum = level.framenum
                        + SECS_TO_FRAMES(1);
                ...
                if (current_player->health <= current_player->dmg) {
                    gi.sound(current_player, CHAN_VOICE,
                            gi.soundindex("player/drown1.wav"), 1, ATTN_NORM,
                            0);
                } else if (genrand_int32() & 1) {
                    gi.sound(current_player, CHAN_VOICE, soundcache[SND_GURP1],
                            1, ATTN_NORM, 0);
                } else {
                    gi.sound(current_player, CHAN_VOICE, soundcache[SND_GURP2],
                            1, ATTN_NORM, 0);
                }
    ...
    if (waterlevel
        ...
        if (current_player->watertype & CONTENTS_LAVA) {
            ...
            //FIXME: ugly hack to a void sizzle damage being multiplied based
            // on server framenum, can be biased depending on what frame the
            // client entered the volume
            if ((level.framenum % (int) (0.1f * SERVER_FPS)) == 0) {
...
void G_SetClientFrame(edict_t *ent) {
    ...
    if (ent->s.frame != ent->client->last_model_frame) {
        // the frame was changed by a weapon fire, jump or other event that
        // occured between game frames, fix up the animations to run at 10 hz
...
void ClientEndServerFrame(edict_t *ent) {
    ...
    // Unfortunately ugly technique for handling kickangles, since we don't
    // know if another kick will occur in the next frame (eg weapon kicks),
    // we have to send the same kickangles for x frames so the client will
    // interpolate between old and cur or you get unsightly view shifting.
    // This also unfortunately negates the effect of view kicks added during
    // the time the kickangles are being duplicated (eg falling / pain kicks)
    ...
    if (ent->client->showscores
            && ((!(level.realframenum % SECS_TO_FRAMES(3))
                    && tdm_match_status != MM_WARMUP
                    && tdm_match_status != MM_TIMEOUT)
                    || (!(level.realframenum % SECS_TO_FRAMES(1.5f))
                            && (tdm_match_status == MM_WARMUP
                                    || tdm_match_status == MM_TIMEOUT)))) {
