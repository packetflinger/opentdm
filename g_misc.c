...
/*
 ...
 self->nextthink = level.framenum + SECS_TO_FRAMES(8 + random()*10);
 }
 }
...
void ThrowGib(edict_t *self, char *gibname, int damage, int type) {
    ...
    gib->nextthink = level.framenum + SECS_TO_FRAMES(10 + random()*10);
    gi.linkentity(gib);
}
...
void ThrowHead(edict_t *self, char *gibname, int damage, int type) {
    ...
    self->think = G_FreeEdict;
    self->nextthink = level.framenum + SECS_TO_FRAMES(10 + random()*10);
    gi.linkentity(self);
}
...
void ThrowDebris(edict_t *self, char *modelname, float speed, vec3_t origin) {
    ...
    chunk->nextthink = level.framenum + SECS_TO_FRAMES(5 + random()*5);
    chunk->s.frame = 0;
    chunk->flags = 0;
 ...
 *
 * TOGGLE           only valid for TRIGGER_SPAWN walls this allows
 *                  the wall to be turned on and off
 *
 * START_ON         only valid for TRIGGER_SPAWN walls the wall
 *                  will initially be present
 */
 ...
 * explosion, set damage and it will do a radius explosion of that amount
...
void SP_misc_blackhole(edict_t *ent) {
    ...
    ent->nextthink = level.framenum + 2 * SECS_TO_FRAMES(0.1f);
    gi.linkentity(ent);
}
...
void SP_monster_commander_body(edict_t *self) {
    ...
    self->nextthink = level.framenum + 5 * SECS_TO_FRAMES(0.1f);
}

/**
 * The origin is the bottom of the banner. The banner is 128 tall.
 */
 ...
 *
 */
void misc_viper_bomb_touch(edict_t *self, edict_t *other, cplane_t *plane,
        csurface_t *surf) {
...
void SP_light_mine1(edict_t *ent) {
    ent->movetype = MOVETYPE_NONE;
    ent->solid = SOLID_BBOX;
    ent->s.modelindex = gi.modelindex(
...
void SP_misc_gib_arm(edict_t *ent) {
    ...
    ent->nextthink = level.framenum + SECS_TO_FRAMES(30);
    gi.linkentity(ent);
}
