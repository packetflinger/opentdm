...
void ScoreboardMessage(edict_t *ent, edict_t *killer) {
    ...
    for (i = 0; i < total; i++) {
        ...
        if (tag) {
            Com_sprintf(entry, sizeof(entry), "xv %i yv %i picn %s ", x + 32, y,
                    tag);
            j = strlen(entry);
...
void Cmd_Score_f(edict_t *ent) {
    ...
    // switch between showing oldscore and current score during warmup
    if (ent->client->showoldscores) {
        ent->client->showoldscores = false;
        return;
    }

    if (tdm_match_status < MM_COUNTDOWN && ent->client->showscores
            && old_matchinfo.scoreboard_string[0]) {
        TDM_OldScores_f(ent);
        return;
    }
...
static void G_SetTeamScoreStats(edict_t *ent) {
    if (ent->client->showoldscores) {
        ent->client->ps.stats[STAT_FIRST_TEAM_SCORE] =
                old_matchinfo.scores[TEAM_A];
        ent->client->ps.stats[STAT_SECOND_TEAM_SCORE] =
                old_matchinfo.scores[(TEAM_A % 2) + 1];
    } else {
        ent->client->ps.stats[STAT_FIRST_TEAM_SCORE] = teaminfo[TEAM_A].score;
        ent->client->ps.stats[STAT_SECOND_TEAM_SCORE] = teaminfo[(TEAM_A % 2)
                + 1].score;
    }
}
...
void G_SetStats(edict_t *ent) {
    ...
    if (power_armor_type
            && (!index || ((level.framenum / SECS_TO_FRAMES(0.1f)) & 8))) {	// flash between power armor and other armor icon
    ...
    if (ent->client->quad_framenum > level.framenum) {
        ...
        ent->client->ps.stats[STAT_TIMER] = FRAMES_TO_SECS(
        ...
        ent->client->ps.stats[STAT_TIMER_ICON] =
                ent->client->pers.item_timer_icon[TIMER_ARMOR];
        ent->client->ps.stats[STAT_TIMER] = FRAMES_TO_SECS(
    ...
    if (ent->client->invincible_framenum > level.framenum) {
        ...
        ent->client->ps.stats[stat] = FRAMES_TO_SECS(
                ent->client->invincible_framenum - level.framenum);
    } else if (ent->client->pers.item_timer[TIMER_WEAPON] > level.framenum) {
        ent->client->ps.stats[stat_icon] =
                ent->client->pers.item_timer_icon[TIMER_WEAPON];
        ent->client->ps.stats[stat] = FRAMES_TO_SECS(
    ...
    // selected item
    if (ent->client->selected_item
            == -1|| itemlist[ent->client->selected_item].icon == NULL) {
        ent->client->ps.stats[STAT_SELECTED_ICON] = 0;
    } else {
        ent->client->ps.stats[STAT_SELECTED_ICON] = gi.imageindex(
                itemlist[ent->client->selected_item].icon);
    }
    ...
    if (ent->health <= 0 || tdm_match_status == MM_SCOREBOARD
            || ent->client->showscores || ent->client->pers.menu.active
    ...
    if (tdm_match_status == MM_TIMEOUT) {
        ent->client->ps.stats[STAT_TIMEOUT_STRING_INDEX] =
                CS_TDM_TIMEOUT_STRING;
    } else {
    ...
    if (ent->client->pers.weaponhud && g_weapon_hud->value > 0) {
        ent->client->ps.stats[STAT_WEAPHUD_BULLETS] =
                ent->client->inventory[ITEM_AMMO_BULLETS];
        ent->client->ps.stats[STAT_WEAPHUD_SHELLS] =
                ent->client->inventory[ITEM_AMMO_SHELLS];
        ent->client->ps.stats[STAT_WEAPHUD_GRENADES] =
                ent->client->inventory[ITEM_AMMO_GRENADES];
        ent->client->ps.stats[STAT_WEAPHUD_CELLS] =
                ent->client->inventory[ITEM_AMMO_CELLS];
        ent->client->ps.stats[STAT_WEAPHUD_ROCKETS] =
                ent->client->inventory[ITEM_AMMO_ROCKETS];
        ent->client->ps.stats[STAT_WEAPHUD_SLUGS] =
                ent->client->inventory[ITEM_AMMO_SLUGS];
...
void G_SetSpectatorStats(edict_t *ent) {
    ...
    if (!cl->chase_target) {
        ...
        memcpy(cl->ps.stats, cl->chase_target->client->ps.stats,
                sizeof(cl->ps.stats));
    ...
    if (tdm_match_status == MM_SCOREBOARD || cl->pers.menu.active
            || ent->client->showscores || ent->client->showoldscores
            || ent->client->showmotd) {
