...
qboolean IsFemale(edict_t *ent) {
    ...
    if (!Q_strncasecmp(teaminfo[ent->client->pers.team].skin, "female/", 7)
            || !Q_strncasecmp(teaminfo[ent->client->pers.team].skin, "crakhor/",
                    8)) {
        return true;
    }
    return false;
...
qboolean IsNeutral(edict_t *ent) {
    ...
    //ugly skin checks instead :(
    if (!Q_strncasecmp(teaminfo[ent->client->pers.team].skin, "male/", 5)
            || !Q_strncasecmp(teaminfo[ent->client->pers.team].skin, "cyborg/",
                    7)
            || !Q_strncasecmp(teaminfo[ent->client->pers.team].skin, "female/",
                    7)
            || !Q_strncasecmp(teaminfo[ent->client->pers.team].skin, "crakhor/",
                    8)) {
        return false;
...
void TossClientWeapon(edict_t *self) {
    ...
    if (!((int) (dmflags->value) & DF_QUAD_DROP)) {
        ...
        quad = (self->client->quad_framenum
                > (level.framenum + SECS_TO_FRAMES(1)));
    }
...
void player_die(edict_t *self, edict_t *inflictor, edict_t *attacker,
    ...
    if (!self->deadflag) {
        self->client->respawn_framenum = level.framenum
                + SECS_TO_FRAMES(g_respawn_time->value);
        ...
        // dont run cmd_help, since that putaways menus, explicitly show scores
    ...
    if (self->health < -40) {   // gib
        ...
        if (!self->deadflag) {
            ...
            gi.sound(self, CHAN_VOICE,
                    soundcache[SND_DEATH1 + genrand_int32() % 4], 1, ATTN_NORM,
                    0);
        }
    }
...
/**
 * go to a random point, but NOT points with other players on them
 */
edict_t* SelectRandomDeathmatchSpawnPointAvoidingTelefrag(edict_t *player) {
    ...
    // hard coded since the player may be spawning from spec and has no inherent
    // size
    ...
    gi.dprintf(
            "SelectRandomDeathmatchSpawnPointAvoidingTelefrag: All spawn points occupied, doing team search.\n");
    ...
    for (i = 0; i < level.numspawns; i++) {
        ...
        if (PlayersRangeFromSpot(spawn, &occupier) < 64) {
            ...
            gi.dprintf(
                    "SelectRandomDeathmatchSpawnPointAvoidingTelefrag: Trying to spawn %s next to %s\n",
                    player->client->pers.netname,
                    occupier->client->pers.netname);
            ...
            for (j = 0; j < 4; j++) {
                ...
                if (!tr.allsolid && !tr.startsolid && tr.fraction == 1.0f) {
                    gi.dprintf(
                            "SelectRandomDeathmatchSpawnPointAvoidingTelefrag: Using fake spawn for %s\n",
                            player->client->pers.netname);
            ...
            gi.dprintf(
                    "SelectRandomDeathmatchSpawnPointAvoidingTelefrag: Failed to find space next to %s for %s\n",
                    occupier->client->pers.netname,
                    player->client->pers.netname);
        ...
        gi.dprintf(
                "SelectRandomDeathmatchSpawnPointAvoidingTelefrag: BUG: Spawn that was previously marked occupied became available!\n");
        return spawn;
    }

    gi.dprintf(
            "SelectRandomDeathmatchSpawnPointAvoidingTelefrag: Failed to find ANY suitable spawn point, using random!!\n");
...
/**
 * Go to a random point, but NOT the two points closest to other players, buggy
 * version
 *
 * wision: this one ruins the game on small maps (i.e. with 4 spawn points)
 */
...
edict_t* SelectDeathmatchSpawnPoint(edict_t *player) {
    ...
    if ((tdm_match_status >= MM_PLAYING
            && level.framenum - level.match_start_framenum < SECS_TO_FRAMES(1))
            || (tdm_match_status == MM_WARMUP
                    && level.framenum - level.warmup_start_framenum
                            < SECS_TO_FRAMES(5))) {
    ...
    if ((int) (dmflags->value) & DF_SPAWN_FARTHEST) {
        ...
        if (TDM_Is1V1()) {
            ...
            if (level.numspawns
                    < 6 && ((int)g_1v1_spawn_mode->value & SPAWN_RANDOM_ON_SMALL_MAPS)) {
                return SelectRandomDeathmatchSpawnPoint();
            }
...
void SelectSpawnPoint(edict_t *ent, vec3_t origin, vec3_t angles) {
    ...
    if (!spot) {
        while ((spot = G_Find(spot, FOFS(classname), "info_player_start"))
                != NULL) {
            if (!spot->targetname) {
                break;
            }
...
void ClientBeginDeathmatch(edict_t *ent) {
    ...
    client->pers.item_timer_icon[TIMER_INVULN] = gi.imageindex(
            "p_invulnerability");
    client->pers.item_timer_icon[TIMER_ENVIROSUIT] = gi.imageindex(
            "p_envirosuit");
    client->pers.item_timer_icon[TIMER_REBREATHER] = gi.imageindex(
            "p_rebreather");
...
void ClientUserinfoChanged(edict_t *ent, char *userinfo) {
    ...
    if (strcmp(old_name, s)) {
        if (old_name[0] && tdm_match_status > MM_COUNTDOWN
                && !g_allow_name_change_during_match->value) {
            gi.cprintf(ent, PRINT_HIGH,
                    "You cannot change your name in the middle of the match!\n");
            Info_SetValueForKey(userinfo, "name", old_name);
            G_StuffCmd(ent, "set name \"%s\"\n", old_name);
        } else {
            edict_t *e;

            Q_strncpy(ent->client->pers.netname, s,
...
/**
 * Called when a player drops from the server. Will be called between levels
...
void ClientDisconnect(edict_t *ent) {
    ...
    // join code stuff is preserved in this function, don't ruin ent fields
    // except inuse before then!
...
void G_TouchProjectiles(edict_t *ent, vec3_t start) {
    ...
    for (e = g_edicts + game.maxclients + 1; e < g_edicts + game.maxentities;
            e++) {
        if (!e->inuse) {
            continue;
        }
    ...
    for (i = 0; i < 10; i++) {
        tr = gi.trace(start, ent->mins, ent->maxs, ent->s.origin, ignore,
                CONTENTS_MONSTER);
...
void ClientThink(edict_t *ent, usercmd_t *ucmd) {
    ...
    if (tdm_match_status == MM_SCOREBOARD
            || (tdm_match_status == MM_TIMEOUT && ent->client->pers.team)) {
    ...
    if (vote.active && tdm_match_status == MM_WARMUP
            && (int) g_vote_attention->value >= 3
            && client->resp.vote == VOTE_HOLD) {
        client->ps.pmove.pm_type = PM_FREEZE;
        return;
    }
...
void ClientBeginServerFrame(edict_t *ent) {
    ...
    // frame across multiple server frames.
    ...
    if (ent->client->pers.team && tdm_match_status == MM_WARMUP
            && g_idle_time->value
            &&
            FRAMES_TO_SECS(level.framenum - ent->client->last_activity_frame)
                    > g_idle_time->value) {
        gi.bprintf(PRINT_HIGH,
                "Removing %s from team '%s' due to inactivity.\n",
                ent->client->pers.netname,
    ...
    if (ent->deadflag) {
        ...
        if ((level.framenum > client->respawn_framenum
                ...
                || (level.framenum
                        > client->respawn_framenum
                                - SECS_TO_FRAMES(g_respawn_time->value - 1)
                        && (client->latched_buttons & BUTTON_ATTACK))) {
            respawn(ent);
            client->latched_buttons = 0;
        }
