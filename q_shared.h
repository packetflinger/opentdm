...
//set this to 1 if you have a stupid endian thingy
...
void AngleVectors(vec3_t angles, vec3_t /*@out@*//*@null@*/forward,
    vec3_t /*@out@*//*@null@*/right, vec3_t /*@out@*//*@null@*/up);
...
typedef struct cvar_s {
    char *name;                 // 0x00
    char *string;               // 0x04
    char *latched_string;       // 0x08
    int flags;                  // 0x0c
    qboolean modified;          // 0x10
    float value;                // 0x14
...
// temp entity events
//
// Temp entity events are for things that happen at a location separate from
...
// sound channels
// channel 0 never willingly overrides other channels (1-7) always override a
...
// Entity events are for effects that take place reletive to an existing
