...
void G_StuffCmd(edict_t *e, const char *fmt, ...) {
    ...
    if (e && !e->client->pers.connected) {
        TDM_Error("G_StuffCmd: Bad client %d for '%s'",
                (int) (e - g_edicts - 1), fmt);
    }
 ...
 * entity that was recently freed, because it can cause the client to think
 * the entity morphed into something else instead of being removed and
...
edict_t* G_Spawn(void) {
    ...
    for (i = game.maxclients + 1; i < globals.num_edicts; i++, e++) {
        ...
        if (!e->inuse
                && (e->freed_framenum < SECS_TO_FRAMES(2)
                        || level.framenum - e->freed_framenum
                                > SECS_TO_FRAMES(0.5f))) {
            G_InitEdict(e);
            return e;
        }
...
void G_TouchTriggers(edict_t *ent) {
    ...
    for (i = 0; i < num; i++) {
        ...
        // don't hit a projectile in its "stopped" state
...
qboolean KillBox(edict_t *ent) {
    ...
    if (g_bugs->value < 2) {
        // much more vicious telefrag code, nukes anything that isn't ent
        count = gi.BoxEdicts(ent->absmin, ent->absmax, touch, MAX_EDICTS,
                AREA_SOLID);
        for (i = 0; i < count; i++) {
            if (touch[i] == ent) {
                continue;
            ...
            if (touch[i]->solid == SOLID_NOT
                    || touch[i]->enttype == ENT_BODYQUE) {
                continue;
            }

            if (touch[i]->inuse) {
                T_Damage(touch[i], ent, ent, vec3_origin, ent->s.origin,
