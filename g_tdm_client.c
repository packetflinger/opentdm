...
/**
 * A new player joined mid-match (invite / etc), update teamplayerinfo to
 * reflect this.
 */
void TDM_AddPlayerToMatchinfo(edict_t *ent) {
    ...
    new_teamplayers = gi.TagMalloc(
            sizeof(teamplayer_t) * (current_matchinfo.num_teamplayers + 1),
            TAG_GAME);
    memcpy(new_teamplayers, current_matchinfo.teamplayers,
            sizeof(teamplayer_t) * current_matchinfo.num_teamplayers);

    //setup this client
    TDM_SetupTeamInfoForPlayer(ent,
            new_teamplayers + current_matchinfo.num_teamplayers);
    ...
    for (i = 0; i < current_matchinfo.num_teamplayers; i++) {
        if (current_matchinfo.captains[TEAM_A]
                == current_matchinfo.teamplayers + i) {
            current_matchinfo.captains[TEAM_A] = new_teamplayers + i;
        } else if (current_matchinfo.captains[TEAM_B]
                == current_matchinfo.teamplayers + i) {
        ...
        if (current_matchinfo.teamplayers[i].client) {
            current_matchinfo.teamplayers[i].client->client->resp.teamplayerinfo =
                    new_teamplayers + i;
        }
    }
...
void JoinedTeam(edict_t *ent, qboolean reconnected, qboolean notify) {
    ...
    // switch from spec to player statusbar, if weapon hud is enabled, that'll
    // happen in respawn()
...
void TDM_LeftTeam(edict_t *ent, qboolean notify) {
    ...
    if (tdm_match_status == MM_TIMEOUT && level.tdm_timeout_caller
            ...
            && level.timeout_end_framenum) {
        TDM_ResumeGame();
    }
}

/**
 * Should player be allowed to join a team at this time?
 */
qboolean CanJoin(edict_t *ent, unsigned team) {
    ...
    if (g_gamemode->value == GAMEMODE_1V1 && teaminfo[team].captain) {
        gi.cprintf(ent, PRINT_HIGH,
                "Only one player is allowed on each team in 1v1 mode.\n");
        return false;
    }
    ...
    if (teaminfo[team].locked) {
        ...
        if (!(ent->client->resp.last_invited_by
                && ent->client->resp.last_invited_by->inuse
    ...
    if (g_max_players_per_team->value
            && teaminfo[team].players >= g_max_players_per_team->value) {
...
/**
 * Player hit Spectator menu option or used chase command.
 */
...
/**
 * Select the next page of help from the help menu. If help menu is not open,
 * opens on the first page of help.
 */
...
void TDM_ShowTeamMenu(edict_t *ent) {
    if (ent->client->pers.menu.active) {
        ...
        if (ent->client->pers.votemenu_values.show) {
            PMenu_Close(ent);
            PMenu_Open(ent, ent->client->pers.joinmenu,
                    teamJoinEntries[ent->client->pers.team], MENUSIZE_JOINMENU,
                    false);
        } else {
            PMenu_Close(ent);
        }
...
qboolean TDM_ProcessJoinCode(edict_t *ent, unsigned value) {
    ...
    // Don't allow bypassing of max players setting.
    if (teaminfo[t->team].players >= current_matchinfo.max_players_per_team) {
        return false;
    }

    // Could be using a joincode while chasing, need to fix it here before we
    // possibly overwrite entity
    if (ent->client->chase_target) {
        DisableChaseCam(ent);
    }
    ...
    if (TDM_Is1V1()) {
        ...
        *ent->client = *t->saved_client;
        ...
        memcpy(ent->clusternums, new_entity.clusternums,
                sizeof(ent->clusternums));
    ...
    // no linkentity, we are called as part of PutClientInServer, linking here
    // would telefrag ourselves!
    gi.unlinkentity(ent);
    return true;
}
...
void TDM_Disconnected(edict_t *ent) {
    ...
    // we remove this up here so TDM_LeftTeam doesn't try to resume if we
    // become implicit timeout caller
    TDM_RemoveStatsLink(ent);

    if (ent->client->pers.team) {
        if (tdm_match_status >= MM_PLAYING
                && tdm_match_status != MM_SCOREBOARD) {
            // do joincode stuff if a team player disconnects
            // save all their client info
            ent->client->resp.teamplayerinfo->saved_client = gi.TagMalloc(
                    sizeof(gclient_t), TAG_GAME);
            *ent->client->resp.teamplayerinfo->saved_client = *ent->client;
            ent->client->resp.teamplayerinfo->saved_entity = gi.TagMalloc(
            ...
            *ent->client->resp.teamplayerinfo->saved_entity = *ent;
            ...
            if (TDM_Is1V1() && g_1v1_timeout->value > 0) {
                ...
                // may have already been set by a player or previous client
                // disconnect
                if (tdm_match_status != MM_TIMEOUT) {
                    edict_t *opponent;

                    // timeout is called implicitly in 1v1 games or the other
                    // player would auto win
                    level.timeout_end_framenum = level.realframenum
                            + SECS_TO_FRAMES(g_1v1_timeout->value);
                    ...
                    gi.bprintf(PRINT_CHAT,
                            "%s disconnected and has %s to reconnect.\n",
                    ...
                    if (opponent) {
                        gi.cprintf(opponent, PRINT_HIGH,
                                "Your opponent has disconnected. You can allow them %s to reconnect, or you can force a forfeit by typing 'win' in the console.\n",
                                TDM_SecsToString(g_1v1_timeout->value));
                    }
    ...
    if (tdm_match_status == MM_WARMUP && teaminfo[TEAM_SPEC].players == 0
...
/**
 * Specs get a slightly different HUD
 */
const char* TDM_CreateSpectatorStatusBar(edict_t *player) {
    ...
    spec_statusbar = va(
            ...
            "if 28 "
                ...
                "yb -180 "
                "stat_string 28 "
            "endif", (int) strlen(teaminfo[TEAM_A].name) * 8,
            teaminfo[TEAM_A].name, (int) strlen(teaminfo[TEAM_B].name) * 8,
            teaminfo[TEAM_B].name, id_x, id_y);
    return spec_statusbar;
}

/**
 * Players get a custom dm_statusbar. It updates every time a new weapon
 * is picked up during a match (shown if `weaponhud` is enabled).
 */
const char* TDM_CreatePlayerDmStatusBar(edict_t *player) {
    ...
    if (SHOWWEAPONHUD(player)) {
        ...
        if (player->client->inventory[ITEM_WEAPON_GRENADELAUNCHER]) {
            ...
            strcat(ammohud,
                    va("yv %d num 3 %d ", hud_y, STAT_WEAPHUD_GRENADES));
            hud_y += 25;
    ...
    dm_statusbar = va(
            ...
            "%s%s", (int) strlen(teaminfo[TEAM_A].name) * 8,
            teaminfo[TEAM_A].name, (int) strlen(teaminfo[TEAM_B].name) * 8,
            teaminfo[TEAM_B].name, id_x, id_y, weaponhud, ammohud);
    return dm_statusbar;
}

/**
 * Send status bar config string to a specific player
 */
...
qboolean TDM_ParsePlayerConfigLine(char *line, int line_number, void *param) {
    ...
    // No validation is done here to keep things small - these should be
    // validated server-side already. Yes, we're trusting the server not
    // to lie to us.
    if (!strcmp(variable, "teamskin")) {
        ...
        gi.dprintf(
                "Unknown player config variable '%s'. Check you are using the latest version of OpenTDM.\n",
                variable);
    }
    return true;
}
...
void TDM_PlayerConfigDownloaded(tdm_download_t *download, int code, byte *buff,
    ...
    if (buff) {
        if (!TDM_ProcessText((char*) buff, len, TDM_ParsePlayerConfigLine,
            ...
            gi.cprintf(download->initiator, PRINT_HIGH,
                    "Your opentdm.org player config was loaded successfully.\n");
        ...
        if (code == 404) {
            gi.cprintf(download->initiator, PRINT_HIGH,
                    "No player config for your stats_id was found. Please check your stats_id cvar is set properly.\n");
        } else if (code == 400) {
            gi.cprintf(download->initiator, PRINT_HIGH,
                    "A malformed stats_id was found while trying to download your player config. Please check your stats_id cvar is set properly.\n");
        } else {
            gi.cprintf(download->initiator, PRINT_HIGH,
                    "An unspecified error %d occured while trying to download your player config.\n",
                    code);
        }
    }

    // set up the dm_statusbar according the config and send it to the client
...
void TDM_DownloadPlayerConfig(edict_t *ent) {
    ...
    Com_sprintf(ent->client->pers.download.path,
            sizeof(ent->client->pers.download.path), "%s", stats_id);
    ent->client->pers.download.initiator = ent;
    ent->client->pers.download.type = DL_PLAYER_CONFIG;
    Q_strncpy(ent->client->pers.download.name, stats_id,
            sizeof(ent->client->pers.download.name) - 1);
...
qboolean TDM_SetupClient(edict_t *ent) {
    ...
    if (!TDM_ProcessJoinCode(ent, 0)) {
        ...
        if (tdm_match_status == MM_TIMEOUT) {
            if (TDM_Is1V1()) {
                // Only resume if we have 2 players, it's possible both players
                // dropped, and that is a situation we need to be able to handle.
                if (teaminfo[TEAM_A].players == 1
...
void TDM_SetInitialItems(edict_t *ent) {
    ...
    case MM_WARMUP:
        ...
        if (g_gamemode->value == GAMEMODE_ITDM) {
            ...
            //spawn with RL up
            if (!client->pers.last_weapon
                    || client->inventory[ITEM_INDEX(client->pers.last_weapon)]
                            == 0) {
...
/**
 * Find the best player for the id view and return a configstring index that
 * contains their info.
 */
int TDM_GetPlayerIdView(edict_t *ent) {
    ...
    if (ent->client->chase_target) {
        ...
        // trace was unsuccessful, try guessing based on angles
        if (!target) {
            edict_t *who, *best;
            vec3_t dir;
            ...
            for (who = g_edicts + 1; who <= g_edicts + game.maxclients; who++) {
                ...
                //note, we trace twice so we hit water planes
                if (d > bd && visible(ent, who, CONTENTS_SOLID | MASK_WATER)
            ...
            if (best) {
                if (!best->client->pers.team) {
                    TDM_Error(
                            "TDM_GetPlayerIdView: Got a spectator via DotProduct with %d health (%s)",
                            best->health, best->client->pers.netname);
                }
                // allow variable slop based on proximity
                if ((bdistance < 150 && bd > 0.50f)
                        || (bdistance < 250 && bd > 0.90f)
                        || (bdistance < 600 && bd > 0.96f) || bd > 0.98f) {
                    target = best;
                }
            }
        } else if (!target->client->pers.team) {
            TDM_Error(
                    "TDM_GetPlayerIdView: Got a spectator via trace with %d health (%s)",
                    target->health, target->client->pers.netname);
        }
    }
    ...
    if (ent->client->pers.team == target->client->pers.team) {
        show_health_info = true;    // same team, show health info
    } else if (ent->client->pers.team == TEAM_SPEC) {
        if (ent->client->chase_target
                && ent->client->chase_target->client->pers.team
                        == target->client->pers.team) {
            // viewing someone on the same team as our chase target, show
            // health info
    ...
    if (target->client->inventory[ITEM_ITEM_POWER_SCREEN] > 0
            || target->client->inventory[ITEM_ITEM_POWER_SHIELD] > 0) {
    ...
    if (!ignoreConfigStringUpdate) {
        ...
        if (show_health_info) {
            ...
            if (ent->client->pers.config.id_highlight) {
                string = TDM_SetColorText(
                        va("%16s H:%d A:%d%s", target->client->pers.netname,
                                target->health, TDM_GetArmorValue(target),
                                buff));
            } else {
                string = va("%16s H:%d A:%d%s", target->client->pers.netname,
                        target->health, TDM_GetArmorValue(target), buff);
            }
        } else {
            if (ent->client->pers.config.id_highlight) {
                string = TDM_SetColorText(
                        va("%16s", target->client->pers.netname));
            } else {
...
void TDM_UpdateHud(edict_t *ent, qboolean force) {
    ...
    if (!force) {
        ...
        if ((level.framenum - ent->client->last_hud_update)
                < SECS_TO_FRAMES(2.0F)) {
            return;
        }
        // not time yet
