...
void Cmd_Use_f(edict_t *ent) {
    ...
    if (tdm_match_status
            == MM_TIMEOUT|| tdm_match_status == MM_SCOREBOARD || (int)g_gamemode->value == GAMEMODE_ITDM) {
        return;
    }
...
void Cmd_Drop_f(edict_t *ent) {
    ...
    if ((tdm_match_status < MM_PLAYING || tdm_match_status == MM_TIMEOUT
...
void Cmd_Kill_f(edict_t *ent) {
    ...
    if (tdm_match_status == MM_COUNTDOWN || tdm_match_status == MM_TIMEOUT
            || tdm_match_status == MM_SCOREBOARD) {
        return;
    }
...
void Cmd_Wave_f(edict_t *ent) {
    ...
    if (flood_waves->value) {
        ...
        if (i < 0) {
            i = (sizeof(cl->resp.flood_waves_when)
                    / sizeof(cl->resp.flood_waves_when[0])) + i;
        }
        if (cl->resp.flood_waves_when[i]
                && (level.framenum - cl->resp.flood_waves_when[i])
                        < flood_waves_waitdelay->value * (60 * SERVER_FPS)) {
            cl->resp.flood_waves_locktill = level.framenum
                    + (flood_waves_waitdelay->value * (60 * SERVER_FPS));
            gi.cprintf(ent, PRINT_CHAT,
                    "Flood protection:  You can't use waves for %d minutes.\n",
                    (int) (flood_waves_waitdelay->value));
            return;
        }
        cl->resp.flood_waves_whenhead = (cl->resp.flood_waves_whenhead + 1)
                % (sizeof(cl->resp.flood_waves_when)
                        / sizeof(cl->resp.flood_waves_when[0]));
        cl->resp.flood_waves_when[cl->resp.flood_waves_whenhead] =
                level.framenum;
    }
...
void Cmd_Reconnect_IPv6_f(edict_t *ent) {
    ...
    g_features = gi.cvar("g_features", "0", CVAR_LATCH);
    if (!((int)g_features->value & GMF_IPV6_ADDRESS_AWARE)) {
        gi.cprintf(ent, PRINT_HIGH, "IPv6 feature not supported\n");
        return;
    }
...
void Cmd_Say_f(edict_t *ent, qboolean team, qboolean arg0) {
    ...
    if (tdm_match_status != MM_TIMEOUT && tdm_match_status > MM_COUNTDOWN
            && tdm_match_status < MM_SCOREBOARD && !ent->client->pers.team
            && !ent->client->pers.admin && g_chat_mode->value == 1) {
        //Observers can talk only to each other during the match.
        team = true;
    }

    if (team) {
        expandpoint = Com_sprintf(text, sizeof(text), "(%s): ",
    ...
    if (ent->client->pers.team) {
        TDM_MacroExpand(ent, text + expandpoint,
                sizeof(text) - expandpoint - 1);
    }
    ...
    if (flood_msgs->value && (!ent->client->pers.team || !team)) {
        ...
        if (level.realframenum < cl->resp.flood_locktill) {
            gi.cprintf(ent, PRINT_HIGH, "You can't talk for %d more seconds\n",
                    (int) ((cl->resp.flood_locktill - level.realframenum)
                            * FRAMETIME));
            return;
        }
        ...
        if (cl->resp.flood_when[i]
                && (level.realframenum - cl->resp.flood_when[i])
                        < flood_waitdelay->value * SERVER_FPS) {
            cl->resp.flood_locktill = level.realframenum
                    + (flood_waitdelay->value * SERVER_FPS);
            gi.cprintf(ent, PRINT_CHAT,
        ...
        cl->resp.flood_whenhead =
                (cl->resp.flood_whenhead + 1)
                        % (sizeof(cl->resp.flood_when)
                                / sizeof(cl->resp.flood_when[0]));
    ...
    for (j = 1; j <= game.maxclients; j++) {
        ...
        if (IGNORING(other, PLAYERS) && ent->client->pers.team
                && ent != other) {
            continue;
        }
