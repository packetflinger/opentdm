...
void Move_Final(edict_t *ent) {
    ...
    VectorScale(ent->moveinfo.dir, ent->moveinfo.remaining_distance / FRAMETIME,
            ent->velocity);
...
void Move_Begin(edict_t *ent) {
    ...
    frames = floor(
            (ent->moveinfo.remaining_distance / ent->moveinfo.speed)
                    / FRAMETIME);
...
void plat_CalcAcceleratedMove(moveinfo_t *moveinfo) {
    ...
    if ((moveinfo->remaining_distance - accel_dist - decel_dist) < 0) {
        ...
        moveinfo->move_speed = (-2
                + sqrt(4 - 4 * f * (-2 * moveinfo->remaining_distance)))
                / (2 * f);
...
void Think_AccelMove(edict_t *ent) {
    ...
    VectorScale(ent->moveinfo.dir, ent->moveinfo.current_speed * 10,
            ent->velocity);
    ent->nextthink = level.framenum + SECS_TO_FRAMES(0.1f);
    ent->think = Think_AccelMove;
}
...
void plat_hit_top(edict_t *ent) {
    ...
    ent->nextthink = level.framenum + SECS_TO_FRAMES(3);
}
...
void Touch_Plat_Center(edict_t *ent, edict_t *other, cplane_t *plane,
    ...
    if (ent->moveinfo.state == STATE_BOTTOM) {
        ...
        ent->nextthink = level.framenum + SECS_TO_FRAMES(1);
    }
}
...
void rotating_touch(edict_t *self, edict_t *other, cplane_t *plane,
        csurface_t *surf) {
...
void button_wait(edict_t *self) {
    ...
    if (self->moveinfo.wait >= 0) {
        self->nextthink = level.framenum + SECS_TO_FRAMES(self->moveinfo.wait);
...
void door_go_up(edict_t *self, edict_t *activator) {
    ...
    if (self->moveinfo.state == STATE_TOP) {    // reset top wait time
        if (self->moveinfo.wait >= 0) {
            self->nextthink = level.framenum
                    + SECS_TO_FRAMES(self->moveinfo.wait);
        }
        return;
    }
...
void door_touch(edict_t *self, edict_t *other, cplane_t *plane,
        csurface_t *surf) {
    if (!other->client) {
        return;
    }
    ...
    self->touch_debounce_framenum = level.framenum + SECS_TO_FRAMES(5);
...
void SP_func_door(edict_t *ent) {
    ...
    ent->moveinfo.distance = abs_movedir[0] * ent->size[0]
            + abs_movedir[1] * ent->size[1] + abs_movedir[2] * ent->size[2]
            - st.lip;
...
void SP_func_water(edict_t *self) {
    ...
    self->moveinfo.distance = abs_movedir[0] * self->size[0]
            + abs_movedir[1] * self->size[1] + abs_movedir[2] * self->size[2]
            - st.lip;
...
void train_blocked(edict_t *self, edict_t *other) {
    ...
    self->touch_debounce_framenum = level.framenum + SECS_TO_FRAMES(0.5f);
    T_Damage(other, self, self, vec3_origin, other->s.origin, vec3_origin,
            self->dmg, 1, 0, MOD_CRUSH);
}
...
void func_timer_think(edict_t *self) {
    G_UseTargets(self, self->activator);
    self->nextthink = level.framenum
            + SECS_TO_FRAMES(self->wait + crandom() * self->random);
}
...
void SP_func_timer(edict_t *self) {
    ...
    if (self->spawnflags & 1) {
        self->nextthink =
                level.framenum
                        + SECS_TO_FRAMES(
                                1.0f + st.pausetime + self->delay + self->wait + crandom() * self->random);
        self->activator = self;
    }
...
void door_secret_move3(edict_t *self) {
    ...
    self->nextthink = level.framenum + SECS_TO_FRAMES(self->wait);
