...
void VoteMenuUpdate(edict_t *ent, unsigned flags) {
    if (flags & VOTE_MENU_GAMEMODE) {
        ...
        if (!((int) g_vote_mask->value & VOTE_GAMEMODE)) {
            ...
            sprintf(ent->client->pers.votemenu_values.string_gamemode,
                    "Gamemode: %s",
                    gameString[ent->client->pers.votemenu_values.gamemode]);
            ent->client->pers.votemenu[2].text =
                    ent->client->pers.votemenu_values.string_gamemode;
    ...
    if (flags & VOTE_MENU_MAP) {
        if (!ent->client->pers.votemenu_values.map[0]
                || !((int) g_vote_mask->value & VOTE_MAP)) {
            strcpy(ent->client->pers.votemenu_values.string_map,
                    "Map:      disabled");
            ent->client->pers.votemenu[3].SelectFunc = NULL;
        } else {
            sprintf(ent->client->pers.votemenu_values.string_map,
                    "Map:      %-16.16s",
                    ent->client->pers.votemenu_values.map);
            ent->client->pers.votemenu[3].SelectFunc = VoteMenuMap;
        }

        ent->client->pers.votemenu[3].text =
                ent->client->pers.votemenu_values.string_map;
    }

    if (flags & VOTE_MENU_CONFIG) {
        if (!ent->client->pers.votemenu_values.config[0]
                || !g_allow_vote_config->value
                || !((int) g_vote_mask->value & VOTE_CONFIG)) {
            strcpy(ent->client->pers.votemenu_values.string_config,
                    "Config:   disabled");
            ent->client->pers.votemenu[4].SelectFunc = NULL;
        } else {
            sprintf(ent->client->pers.votemenu_values.string_config,
                    "Config:   %-16.16s",
                    ent->client->pers.votemenu_values.config);
            ent->client->pers.votemenu[4].SelectFunc = VoteMenuConfig;
        }

        ent->client->pers.votemenu[4].text =
                ent->client->pers.votemenu_values.string_config;
    }
    ...
    if (flags & VOTE_MENU_TIMELIMIT) {
        if (!((int) g_vote_mask->value & VOTE_TIMELIMIT)) {
            ...
            sprintf(ent->client->pers.votemenu_values.string_timelimit,
                    "Timelimit: %d minutes",
                    ent->client->pers.votemenu_values.timelimit);
            ent->client->pers.votemenu[6].text =
                    ent->client->pers.votemenu_values.string_timelimit;
    ...
    if (flags & VOTE_MENU_OVERTIME) {
        if (!((int) g_vote_mask->value & VOTE_OVERTIME)) {
            ...
            if (ent->client->pers.votemenu_values.overtime == -1) {
                strcpy(ent->client->pers.votemenu_values.string_overtime,
                        "Overtime:  Sudden Death");
            } else if (ent->client->pers.votemenu_values.overtime == 0) {
                strcpy(ent->client->pers.votemenu_values.string_overtime,
                        "Overtime:  Tie Mode");
            } else if (ent->client->pers.votemenu_values.overtime == 1) {
                sprintf(ent->client->pers.votemenu_values.string_overtime,
                        "Overtime:  1 minute");
            } else {
                sprintf(ent->client->pers.votemenu_values.string_overtime,
                        "Overtime:  %d minutes",
                        ent->client->pers.votemenu_values.overtime);
            }

            ent->client->pers.votemenu[7].text =
                    ent->client->pers.votemenu_values.string_overtime;
    ...
    if (flags & VOTE_MENU_POWERUPS) {
        if (!((int) g_vote_mask->value & VOTE_POWERUPS)) {
            ...
            sprintf(ent->client->pers.votemenu_values.string_powerups,
                    "Powerups:  %d",
                    ent->client->pers.votemenu_values.powerups);
            ent->client->pers.votemenu[8].text =
                    ent->client->pers.votemenu_values.string_powerups;
    ...
    if (flags & VOTE_MENU_BFG) {
        if (!((int) g_vote_mask->value & VOTE_WEAPONS)) {
            ...
            sprintf(ent->client->pers.votemenu_values.string_bfg,
                    "BFG:       %d", ent->client->pers.votemenu_values.bfg);
            ent->client->pers.votemenu[9].text =
                    ent->client->pers.votemenu_values.string_bfg;
    ...
    if (flags & VOTE_MENU_KICK) {
        if (!((int) g_vote_mask->value & VOTE_KICK)) {
            ...
            if (victim == NULL) {
                strcpy(ent->client->pers.votemenu_values.string_kick,
                        "Kick: ---");
            } else {
                sprintf(ent->client->pers.votemenu_values.string_kick,
                        "Kick: %s", victim->client->pers.netname);
            }

            ent->client->pers.votemenu[11].text =
                    ent->client->pers.votemenu_values.string_kick;
    ...
    if (flags & VOTE_MENU_BUGS) {
        static const char *bugsString[] = { "all bugs fixed",
                "serious bugs fixed", "default q2 behavior" };
...
void VoteMenuGameMode(edict_t *ent) {
    if (ent->client->pers.votemenu_values.decrease) {
        ent->client->pers.votemenu_values.gamemode =
                (ent->client->pers.votemenu_values.gamemode + 2) % 3;
    } else {
        ent->client->pers.votemenu_values.gamemode =
                (ent->client->pers.votemenu_values.gamemode + 1) % 3;
    }
...
void VoteMenuTimelimit(edict_t *ent) {
    if (ent->client->pers.votemenu_values.decrease) {
        ent->client->pers.votemenu_values.timelimit =
                ((ent->client->pers.votemenu_values.timelimit + 20) % 30) + 5;
    } else {
        ent->client->pers.votemenu_values.timelimit =
                (ent->client->pers.votemenu_values.timelimit % 30) + 5;
    }
...
void VoteMenuOvertime(edict_t *ent) {
    if (ent->client->pers.votemenu_values.decrease) {
        ...
        ent->client->pers.votemenu_values.overtime =
                ((ent->client->pers.votemenu_values.overtime + 7) % 7) - 1;
    } else {
        // we need to cycle from -1 up to 5
        ent->client->pers.votemenu_values.overtime =
                ((ent->client->pers.votemenu_values.overtime + 2) % 7) - 1;
    }
...
void VoteMenuPowerups(edict_t *ent) {
    ...
    ent->client->pers.votemenu_values.powerups =
            (ent->client->pers.votemenu_values.powerups + 1) % 2;
...
void VoteMenuBFG(edict_t *ent) {
    ...
    ent->client->pers.votemenu_values.bfg =
            (ent->client->pers.votemenu_values.bfg + 1) % 2;
...
void VoteMenuKick(edict_t *ent) {
    ...
    if (ent->client->pers.votemenu_values.decrease) {
        ...
        while (victim > g_edicts
                && (!victim->inuse || victim == ent
                        || victim->client->pers.admin
                        || victim->client->pers.mvdclient)) {
            victim--;
        }
    } else {
        if (ent->client->pers.votemenu_values.kick == NULL) {
        ...
        while (victim <= g_edicts + game.maxclients
                && (!victim->inuse || victim == ent
                        || victim->client->pers.admin
                        || victim->client->pers.mvdclient)) {
            victim++;
...
void VoteMenuChat(edict_t *ent) {
    // only 2 values.. no need for decreasing
    ent->client->pers.votemenu_values.chat =
            (ent->client->pers.votemenu_values.chat + 1) % 2;
...
void VoteMenuBugs(edict_t *ent) {
    if (ent->client->pers.votemenu_values.decrease) {
        ent->client->pers.votemenu_values.bugs =
                (ent->client->pers.votemenu_values.bugs + 2) % 3;
    } else {
        ent->client->pers.votemenu_values.bugs =
                (ent->client->pers.votemenu_values.bugs + 1) % 3;
    }
...
void OpenVoteMenu(edict_t *ent) {
    ...
    if (tdm_match_status == MM_PLAYING
            && ((int) g_vote_mask->value & VOTE_TIMELIMIT)) {
        ent->client->pers.votemenu_values.timelimit = ((int) g_match_time->value
                / 60);
        memcpy(ent->client->pers.votemenu, votemenu, sizeof(votemenu));
        VoteMenuUpdate(ent, VOTE_MENU_TIMELIMIT);

        PMenu_Close(ent);
        PMenu_Open(ent, ent->client->pers.votemenu, 0, MENUSIZE_JOINMENU,
                false);
        return;
    ...
    if (g_tie_mode->value == 1) {
        ent->client->pers.votemenu_values.overtime = ((int) g_overtime->value
                / 60);
