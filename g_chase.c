...
void DisableChaseCam(edict_t *ent) {
    ...
    if ((teaminfo[TEAM_A].speclocked || teaminfo[TEAM_B].speclocked)
        ...
        gi.centerprintf(ent,
                "Can't freefloat while teams are locked against spectators.");
        GetChaseTarget(ent);
    }
}
...
void UpdateChaseCam(edict_t *ent) {
    ...
    if (!targ->inuse || !targ->client->pers.team
            || (teaminfo[targ->client->pers.team].speclocked
                    && !ent->client->pers.specinvite[targ->client->pers.team]
                    && !ent->client->pers.admin)) {
        ChaseNext(ent);
    ...
    if (ent->client->chase_mode == CHASE_EYES) {
        ...
        if (!(game.server_features & GMF_CLIENTNUM)) {
            ...
            trace = gi.trace(targorigin, vec3_origin, vec3_origin, goal, targ,
                    MASK_SOLID);
...
void ChaseNext(edict_t *ent) {
    ...
    do {
        ...
        if (e->client->pers.team
                && (ent->client->pers.admin
                        || !teaminfo[e->client->pers.team].speclocked
                        || ent->client->pers.specinvite[e->client->pers.team])) {
            break;
        }
...
void GetChaseTarget(edict_t *ent) {
    ...
    for (i = 1; i <= game.maxclients; i++) {
        ...
        if (other->inuse && other->client->pers.team
                && (ent->client->pers.admin
                        || !teaminfo[other->client->pers.team].speclocked
                        || ent->client->pers.specinvite[other->client->pers.team])) {
            ent->client->chase_mode = CHASE_EYES;
            SetChase(ent, other);
            UpdateChaseCam(ent);
            return;
