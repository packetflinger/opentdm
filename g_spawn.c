        ...
        if (!(f->flags & FFL_NOSPAWN) && !Q_stricmp(f->name, key)) {// found it
            ...
            case F_VECTOR:
                if (sscanf(value, "%f %f %f", &vec[0], &vec[1], &vec[2]) != 3) {
                    gi.dprintf(
                            "WARNING: ED_ParseField: Couldn't parse F_VECTOR %s, using (0, 0, 0)\n",
                            value);
                    VectorClear(vec);
                }
...
const char* ED_ParseEdict(const char *data, edict_t *ent) {
    ...
    while (1) {
        ...
        com_token = COM_Parse(&data); // parse value
        if (!data) {
            gi.error("ED_ParseEntity: EOF without closing brace");
        }
        if (com_token[0] == '}') {
    ...
    while (1) {
        ...
        if (respawn) {
            ...
            if (ent != world) {
                ...
                if (ent->enttype == ENT_DOOR_TRIGGER
                        || ent->enttype == ENT_PLAT_TRIGGER) {
                    TDM_Error(
                            "ParseEntityString: Trying to reuse an already spawned ent!");
                }
                G_FreeEdict(ent);
                G_InitEdict(ent);
            }
        ...
        if (!respawn) {
            if (num_spawned_entities == MAX_EDICTS * 4) {
                TDM_Error(
                        "Exceeded MAX_EDICTS * 4 entities during entity string parse");
            }
            spawned_entities[num_spawned_entities++] = ent;
        } else {
            ...
            if (tdm_match_status < MM_PLAYING
                    && (ent->solid == SOLID_BBOX || ent->solid == SOLID_BSP)) {
                ...
                count = gi.BoxEdicts(ent->absmin, ent->absmax, touch,
                ...
                for (j = 0; j < count; j++) {
                    ...
                    if (touch[j]->solid == SOLID_NOT
                            || touch[j]->enttype == ENT_BODYQUE) {
                        continue;
                    }
                    ...
                    if (touch[j]->inuse) {
                        T_Damage(touch[j], ent, ent, vec3_origin, ent->s.origin,
...
void SP_worldspawn(edict_t *ent) {
    ...
    if (ent->message && ent->message[0]) {
        ...
        Q_strncpy(level.level_name, level.mapname,
                sizeof(level.level_name) - 1);
    }
    ...
    gi.configstring(CS_SKYAXIS,
            va("%f %f %f", st.skyaxis[0], st.skyaxis[1], st.skyaxis[2]));
    gi.configstring(CS_CDTRACK, va("%i", ent->sounds));
    gi.configstring(CS_MAXCLIENTS, va("%i", game.maxclients));

    // Sent unicast to each player during match but also globally to
    // not mess with gtv/etc.
    ...
    gi.soundindex("items/respawn1.wav");    // gibs
