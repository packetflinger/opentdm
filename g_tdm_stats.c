...
void TDM_WeaponFired(edict_t *ent) {
    ...
    if (!ent->client->resp.teamplayerinfo) {
        TDM_Error(
                "TDM_WeaponFired: Trying to track stats but no teamplayerinfo for client %s",
...
/**
 * Begin a sequence of possibly multiple damage, we only want to count one
 * though for the weapon. We are called like so - TDM_BeginDamage ->
 * TDM_Damage(multiple) -> TDM_EndDamage (stop tracking)
 */
...
void TDM_Damage(edict_t *ent, edict_t *victim, edict_t *inflictor, int damage) {
    ...
    if (!victim->client->resp.teamplayerinfo) {
        TDM_Error(
                "TDM_Damage: Trying to track stats but no teamplayerinfo for victim %s, attacker %s",
                victim->client->pers.netname, ent->client->pers.netname);
    }

    if (!ent->client->resp.teamplayerinfo) {
        TDM_Error(
                "TDM_Damage: Trying to track stats but no teamplayerinfo for attacker %s, victim %s",
...
void TDM_Killed(edict_t *attacker, edict_t *victim, int mod) {
    ...
    if (!victim->client->resp.teamplayerinfo) {
        TDM_Error(
                "TDM_Killed: Trying to track stats but no teamplayerinfo for victim %s, attacker %s",
                victim->client->pers.netname, attacker->client->pers.netname);
    }

    if (!attacker->client->resp.teamplayerinfo) {
        TDM_Error(
                "TDM_Killed: Trying to track stats but no teamplayerinfo for attacker %s, victim %s",
    ...
    if (gi.argc() < 2) {
        ...
        if (!victim) {
            ...
            gi.cprintf(ent, PRINT_HIGH,
                    "No match for '%s'. Players in the match: %s\n", gi.args(),
                    participants);
            return victim;
        }
    }
...
int TDM_GetTeamFromMatchInfo(edict_t *ent, matchinfo_t *matchinfo) {
    ...
    if (gi.argc() < 2) {
        ...
        if (ent->client->chase_target
                && ent->client->chase_target->client->resp.teamplayerinfo) {
        ...
        gi.cprintf(ent, PRINT_HIGH,
                "Team '%s' not found. Use either the name of a player to look up their team, or use team 1 or 2.\n",
                gi.args());
    }
...
/**
 * Returns true if someone in a match is trying to view stats of the other
 * team. Prints a message to ent if they are doing such.
 */
qboolean TDM_StatCheatCheck(edict_t *ent, matchinfo_t *info, unsigned team) {
    if (info != &old_matchinfo && tdm_match_status >= MM_PLAYING
            && tdm_match_status != MM_SCOREBOARD && ent->client->pers.team
            && ent->client->pers.team != team) {
        gi.cprintf(ent, PRINT_HIGH,
                "You can only see stats for the other team after the match has finished.\n");
        return true;
    }
    return false;
}
...
char* TDM_BuildWeaponsStatsString(edict_t *ent, matchinfo_t *m_info,
    ...
    basic =
            (tdm_match_status >= MM_PLAYING && tdm_match_status != MM_SCOREBOARD)
                    && ent->client->pers.team && (m_info == &current_matchinfo);
    ...
    for (i = TDMG_BLASTER; i < TDMG_MAX; i++) {
        ...
        if (!p_info->shots_fired[i] && !p_info->items_collected[tdmg_weapons[i]]
                && i > TDMG_BLASTER && !p_info->damage_received[i]
                && m_info->game_mode != GAMEMODE_ITDM
        ...
        if (basic) {
            ...
            sprintf(missed, "%4d",
                    m_info->item_spawn_count[tdmg_weapons[i]]
                            - p_info->items_collected[tdmg_weapons[i]]);
        }
        ...
        if (p_info->shots_fired[i]) {
            sprintf(acc, "%3.0f%%",
                    ceil(
                            ((float) p_info->shots_hit[i]
                                    / (float) p_info->shots_fired[i]) * 100));
        ...
        strcat(stats,
                va("%16.16s | %s %s  %s %s %s %4d %s\n", item->pickup_name, acc,
                        frags, deaths, dealt, recvd,
                        p_info->items_collected[tdmg_weapons[i]], missed));
    }
...
char* TDM_BuildTeamWeaponsStatsString(edict_t *ent, matchinfo_t *m_info,
    ...
    static char stats[1400];
    unsigned i, j;
    int frags_i, deaths_i, dealt_i, recvd_i, picked_i, total_hit_i,
            total_shots_i;
    char frags[6], deaths[6], dealt[6], recvd[6], missed[6], acc[6];
    teamplayer_t *p_info;
    qboolean basic;
    ...
    for (i = TDMG_BLASTER; i < TDMG_MAX; i++) {
        ...
        frags_i = deaths_i = dealt_i = recvd_i = picked_i = total_hit_i =
                total_shots_i = 0;
        skip = true;
        ...
        if (basic) {
            ...
            sprintf(missed, "%4d",
                    m_info->item_spawn_count[tdmg_weapons[i]] - picked_i);
        }

        // display these stats only for weapons where player fired something
        if (total_shots_i) {
            sprintf(acc, "%3.0f%%",
                    ceil(((float) total_hit_i / (float) total_shots_i) * 100));
        ...
        strcat(stats,
                va("%16.16s | %s %s  %s %s %s %4d %s\n", item->pickup_name, acc,
                        frags, deaths, dealt, recvd, picked_i, missed));
    }
...
char* TDM_BuildItemsStatsString(edict_t *ent, matchinfo_t *m_info,
    ...
    for (i = 1; i < game.num_items; i++) {
        ...
        if (basic) {
            ...
            sprintf(missed, "%4d",
                    m_info->item_spawn_count[i] - p_info->items_collected[i]);
        }

        strcat(stats,
                va("%16.16s |    - %s  %s %s %s %4d %s\n",
                        ((i == ITEM_ITEM_HEALTH) ?
                                va("MegaHealth") : item->pickup_name), frags,
                        deaths, dealt, recvd, p_info->items_collected[i],
                        missed));
    }
    ...
    for (i = 1; i < game.num_items; i++) {
        ...
        strcat(stats,
                va("%16.16s |    - %s  %s %s %s %4d %s\n",
                        ((i == ITEM_ITEM_HEALTH) ?
                                va("MegaHealth") : item->pickup_name), frags,
                        deaths, dealt, recvd, picked_i, missed));
    }
...
void TDM_ItemsStats_f(edict_t *ent, matchinfo_t *m_info, teamplayer_t *p_info) {
    ...
    strcat(stats,
            va("%s\n",
                    TDM_SetColorText(
                            va("Acc. Kills Deaths Dealt Recvd Pick Miss"))));
    strcat(stats,
            va("-----------------+----------------------------------------\n"));
...
void TDM_GeneralStats_f(edict_t *ent, matchinfo_t *m_info, teamplayer_t *p_info,
    ...
    team_kills = team_deaths = team_suicides = team_team_kills =
            team_telefrags = 0;
    team_dealt = team_recvd = team_t_dealt = team_t_recvd = p_recvd = p_dealt =
            0;
    stats[0] = 0;
    ...
    strcat(stats,
            va("  | %s\n",
                    TDM_SetColorText(
                            va(
                                    "Kills Dths Sui TKs Tele Dealt Recvd TDealt TRecvd"))));
    strcat(stats,
            va("-------+--------------------------------------------------\n"));
    ...
    if (p_info) {
        ...
        strcat(stats,
                va("Player | %5d %4d %3d %3d %4d %5d %5d %6d %6d\n",
                        p_info->enemy_kills, p_info->deaths, p_info->suicides,
                        p_info->team_kills, p_info->telefrags, p_dealt, p_recvd,
                        p_info->team_dealt, p_info->team_recvd));
    }
    ...
    if (!m_info->is1v1) {
        ...
        strcat(stats,
                va("Team   | %5d %4d %3d %3d %4d %5d %5d %6d %6d\n", team_kills,
                        team_deaths, team_suicides, team_team_kills,
                        team_telefrags, team_dealt, team_recvd, team_t_dealt,
                        team_t_recvd));
    }
    strcat(stats, va("\n"));
    gi.cprintf(ent, PRINT_HIGH, "%s", stats);
...
/**
 * Shows shitloads of useless information.
 *
 * Note it isn't hard to overflow the 1024 byte cprintf limit, which is why
 * this is split into multiple cprints. On non-R1Q2 servers, this will probably
 * cause overflows!
 */
...
char* TDM_BuildTopBottomShotsString(edict_t *ent, int team, qboolean top_shots) {
    ...
    for (i = TDMG_BLASTER; i < TDMG_MAX; i++) {
        ...
        strcat(stats,
                va("%16.16s | %5.1f%% %4d %5d %5d %6d %s\n", item->pickup_name,
                        accuracy * 100, hits, shots, frags, deaths, player));
    }
...
void TDM_SetupTeamInfoForPlayer(edict_t *ent, teamplayer_t *info) {
    ...
    if (code[0]) {
        ...
        if (!value || TDM_FindTeamplayerForJoinCode(value)) {
            gi.cprintf(ent, PRINT_HIGH,
                    "Your preferred join code could not be set.\n");
        } else {
            info->joincode = value;
        }
    }
...
void TDM_SetupMatchInfoAndTeamPlayers(void) {
    ...
    current_matchinfo.num_teamplayers = teaminfo[TEAM_A].players
            + teaminfo[TEAM_B].players;
    current_matchinfo.teamplayers = gi.TagMalloc(
            current_matchinfo.num_teamplayers * sizeof(teamplayer_t), TAG_GAME);
    ...
    for (i = 0, ent = g_edicts + 1; ent <= g_edicts + game.maxclients; ent++) {
        ...
        if (!ent->inuse) {
            if (ent->client->pers.connected) {
                if (!(game.server_features & GMF_WANT_ALL_DISCONNECTS)) {
                    gi.dprintf(
                            "TDM_SetupMatchInfoAndTeamPlayers: Unclean disconnect of client %d.\n",
                            (int) (ent - g_edicts - 1));
        ...
        if (ent->client->resp.teamplayerinfo) {
            TDM_Error(
                    "TDM_SetupMatchInfoAndTeamPlayers: should not have teamplayerinfo");
        }
    ...
    if (teaminfo[TEAM_A].captain) {
        current_matchinfo.captains[TEAM_A] =
                teaminfo[TEAM_A].captain->client->resp.teamplayerinfo;
    }

    if (teaminfo[TEAM_B].captain) {
        current_matchinfo.captains[TEAM_B] =
                teaminfo[TEAM_B].captain->client->resp.teamplayerinfo;
    }
...
qboolean TDM_IsTrackableItem(edict_t *ent) {
    if (!ent->item) {
        TDM_Error("TDM_IsTrackableItem: Got entity %d with no item",
                (int) (ent - g_edicts));
    }
...
void TDM_ItemSpawned(edict_t *ent) {
    // as per players requests, only items that are grabbed count towards the
    // total
    return;
...
void TDM_ItemGrabbed(edict_t *ent, edict_t *player) {
    ...
    if (!player->client->resp.teamplayerinfo) {
        TDM_Error("TDM_ItemGrabbed: No teamplayerinfo for client %d",
                (int) (player - g_edicts - 1));
    }
