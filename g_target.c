 ...
 *  on/off. Multiple identical looping sounds will just increase volume without
 *  any speed cost.
 */
...
void SP_target_speaker(edict_t *ent) {
    ...
    if (!strstr(st.noise, ".wav")) {
        ...
        Q_strncpy(buffer, st.noise, sizeof(buffer) - 1);
    }
...
void use_target_changelevel(edict_t *self, edict_t *other, edict_t *activator) {
    ...
    if (!((int) dmflags->value & DF_ALLOW_EXIT) && other != world) {
        T_Damage(other, self, self, vec3_origin, other->s.origin, vec3_origin,
                10 * other->max_health, 1000, 0, MOD_EXIT);
        return;
    }
 ...
 *
 * For gibs:
 *      Set direction if you want it moving and speed how fast it
 ...
 * the same unit.  It is OK to check multiple triggers.  Message, delay,
 * target, and killtarget also work.
 */
void trigger_crosslevel_trigger_use(edict_t *self, edict_t *other,
        edict_t *activator) {
 ...
 * triggers are checked, all must be true.  Delay, target and killtarget also
 * work.
 *
...
void target_earthquake_use(edict_t *self, edict_t *other, edict_t *activator) {
    self->damage_debounce_framenum = level.framenum
            + SECS_TO_FRAMES(self->count);
    self->nextthink = level.framenum + SECS_TO_FRAMES(0.1f);
    self->activator = activator;
